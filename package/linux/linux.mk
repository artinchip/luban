...
LINUX_ENABLE_TARBALL = NO
LINUX_ENABLE_PATCH = NO
LINUX_ADD_LINUX_HEADERS_DEPENDENCY = NO
LINUX_GEN_PREBUILT_TARBALL = NO
LINUX_USE_PREBUILT_TARBALL = NO
...
ifeq ($(BR2_LINUX_KERNEL_IMAGEBZ2),y)
LINUX_DEPENDENCIES += host-bzip2
...
ifeq ($(BR2_LINUX_KERNEL_BZIMAGE),y)
...
else ifeq ($(BR2_LINUX_KERNEL_IMAGEBZ2),y)
LINUX_IMAGE_NAME = Image.bz2
else ifeq ($(BR2_LINUX_KERNEL_IMAGEGZ),y)
...
define LINUX_APPLY_LOCAL_PATCHES
	$(Q)for p in $(filter-out ftp://% http://% https://%,$(LINUX_PATCHES)) ; do \
		if test -d $$p ; then \
...
define LINUX_KCONFIG_FIXUP_CMDS
	...
	$(if $(strip $(LINUX_COMPRESSION_OPT_y)),
	...
	$(if $(BR2_TARGET_ROOTFS_INITRAMFS),
		...
		touch $(BINARIES_DIR)/rootfs.cpio.gz
		$(call KCONFIG_SET_OPT,CONFIG_INITRAMFS_SOURCE,"$${BR_BINARIES_DIR}/rootfs.cpio.gz")
...
define LINUX_BUILD_DTB
	$(Q)$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(@D) $(LINUX_DTBS)
endef
...
define LINUX_BUILD_CMDS
	...
	$(foreach dts,$(call qstrip,$(BR2_LINUX_KERNEL_CUSTOM_DTS_PATH)), \
		$(Q)cp -f $(dts) $(LINUX_ARCH_PATH)/boot/dts/
	)
	$(Q)$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(@D) all
	$(Q)$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(@D) $(LINUX_TARGET_NAME)
	$(Q)$(LINUX_BUILD_DTB)
	$(Q)$(LINUX_APPEND_DTB)
endef
...
# build process.
define LINUX_INSTALL_IMAGE
	$(Q)$(INSTALL) -m 0644 -D $(LINUX_IMAGE_PATH) $(1)/$(notdir $(LINUX_IMAGE_NAME))
	$(Q)$(INSTALL) -m 0644 -D $(LINUX_ARCH_PATH)/boot/Image $(1)/Image
	$(Q)$(INSTALL) -m 0644 -D $(LINUX_DIR)/vmlinux $(1)/vmlinux
	$(Q)cd $(1) && echo In $(1) && ls -og --time-style=iso \
		Image $(notdir $(LINUX_IMAGE_NAME)) vmlinux
	$(Q)cd $(1) && echo The section sizes of vmlinux: && size vmlinux
endef
endif
...
define LINUX_INSTALL_KERNEL_DOT_CONFIG_TO_TARGET
	$(INSTALL) -m 0644 -D $(@D)/.config $(TARGET_DIR)/etc/config/linux.config
endef
endif

# Installing dtc (device tree compiler) as host tool, if selected
define LINUX_INSTALL_HOST_TOOLS
	$(Q)if grep -q "CONFIG_DTC=y" $(@D)/.config; then \
...
define LINUX_INSTALL_TARGET_CMDS
	...
	$(LINUX_INSTALL_KERNEL_DOT_CONFIG_TO_TARGET)
...
define LINUX_RUN_DEPMOD
	$(Q)if test -d $(TARGET_DIR)/lib/modules/$(LINUX_VERSION_PROBED) \
...
linux-rebuild-with-initramfs:
	...
	# Build the kernel.
	$(Q)$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(LINUX_DIR) all
	$(LINUX_APPEND_DTB)
