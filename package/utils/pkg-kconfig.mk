...
define kconfig-package-savedefconfig
	$(Q)$($(1)_MAKE_ENV) $($(1)_MAKE) -C $($(1)_DIR) \
		$(PKG_KCONFIG_COMMON_OPTS) $($(1)_KCONFIG_OPTS) savedefconfig
	$(if $(shell cat $($(1)_DIR)/.defconfig_file), \
		cp -f $($(1)_DIR)/defconfig $($(1)_SRCDIR)/$(shell cat $($(1)_DIR)/.defconfig_file))
endef
...
define kconfig-package-merge-config
	@$(call MESSAGE,"kconfig-package-merge-config")
	@rm -f $($(1)_DIR)/.defconfig_file
	@mkdir -p $($(1)_DIR)
	@touch $($(1)_DIR)/.defconfig_file
	$(Q)$(if $($(1)_KCONFIG_DEFCONFIG),\
		$($(1)_KCONFIG_MAKE) $($(1)_KCONFIG_DEFCONFIG),\
		$(INSTALL) -m 0644 -D $($(1)_KCONFIG_FILE) $(2))
	$(Q)tools/support/kconfig/merge_config.sh -m -O $(dir $(2)) $(2) $(3)
...
$(2)_KCONFIG_MAKE = \
	$$($(2)_MAKE_ENV) $$($(2)_MAKE) -C $$($(2)_SRCDIR) O=$$($(2)_DIR) \
...
ifeq ($$($(2)_USE_PREBUILT_TARBALL),NO)
# Before running configure, the configuration file should be present and fixed
$$($(2)_TARGET_CONFIGURE): $$($(2)_DIR)/.stamp_kconfig_fixup_done

# Force olddefconfig again on -reconfigure
$(HIDDEN_PREFIX)-$(1)-clean-for-reconfigure: $(HIDDEN_PREFIX)-$(1)-clean-kconfig-for-reconfigure

$(HIDDEN_PREFIX)-$(1)-clean-kconfig-for-reconfigure:
	rm -f $$($(2)_DIR)/$$($(2)_KCONFIG_STAMP_DOTCONFIG)
endif # $(2)_USE_PREBUILT_TARBALL
...
#
$$(addprefix $(1)-,$$($(2)_KCONFIG_EDITORS)): $(1)-%: $$($(2)_DIR)/.kconfig_editor_% $(1)-savedefconfig
$$($(2)_DIR)/.kconfig_editor_%: PKG=$(2)
$$($(2)_DIR)/.kconfig_editor_%: $$($(2)_DIR)/.stamp_kconfig_fixup_done
	$(Q)$$($(2)_CONFIGURATOR_MAKE_ENV) $$($(2)_MAKE) -C $$($(2)_DIR) \
		$$(PKG_KCONFIG_COMMON_OPTS) $$($(2)_KCONFIG_OPTS) $$(*)
	$(Q)rm -f $$($(2)_DIR)/.stamp_{kconfig_fixup_done,configured,built}
	$(Q)rm -f $$($(2)_DIR)/.stamp_{target,staging,images}_installed
	$$($(2)_FIXUP_DOT_CONFIG)
...
#
$(HIDDEN_PREFIX)-$(1)-check-configuration-done:
...
$(1)-savedefconfig: $(HIDDEN_PREFIX)-$(1)-check-configuration-done
...
$(1)-update-config: $(HIDDEN_PREFIX)-$(1)-check-configuration-done
...
$(1)-savedefconfig: $(1)-update-config
endif
...
# around.
$(HIDDEN_PREFIX)-$(1)-diff-config: $(HIDDEN_PREFIX)-$(1)-check-configuration-done
...
host-kconfig-package = $(call inner-kconfig-package,host-$(pkgname),$(call UPPERCASE,host-$(pkgname)),$(call UPPERCASE,$(pkgname)),host)
