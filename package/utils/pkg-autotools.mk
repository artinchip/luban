...
define CONFIG_UPDATE
	for file in config.guess config.sub; do \
		for i in $$(find $(1) -name $$file); do \
			cp tools/support/gnuconfig/$$file $$i; \
		done; \
	done
endef
...
define LIBTOOL_PATCH_HOOK
	...
	$(Q)for i in `find $($(PKG)_DIR) -name ltmain.sh`; do \
		...
		if test $${ltmain_version} = '1.5'; then \
			patch -i tools/support/libtool/buildroot-libtool-v1.5.patch $${i}; \
		elif test $${ltmain_version} = "2.2"; then\
			patch -i tools/support/libtool/buildroot-libtool-v2.2.patch $${i}; \
		elif test $${ltmain_version} = "2.4"; then\
			if test $${ltmain_patchlevel:-0} -gt 2; then\
				patch -i tools/support/libtool/buildroot-libtool-v2.4.4.patch $${i}; \
			else \
				patch -i tools/support/libtool/buildroot-libtool-v2.4.patch $${i}; \
			fi \
		fi \
	done
endef
...
define CONFIGURE_FIX_POWERPC64_HOOK
	...
	tools/support/scripts/fix-configure-powerpc64.sh $($(PKG)_DIR)
endef
...
define $(2)_INSTALL_TARGET_CMDS
	$$(TARGET_MAKE_ENV) $$($$(PKG)_MAKE_ENV) $$($$(PKG)_MAKE) $$($$(PKG)_INSTALL_TARGET_OPTS) -C $$($$(PKG)_DIR)
endef
endif
