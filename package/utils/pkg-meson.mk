...
define $(2)_CONFIGURE_CMDS
	...
	sed -e 's%@TARGET_CROSS@%$$(TARGET_CROSS)%g' \
	    ...
	    package/third-party/meson/cross-compilation.conf.in \
	    > $$($$(PKG)_BUILDDIR)/build/cross-compilation.conf
	PATH=$$(BR_PATH) \
	CC_FOR_BUILD="$$(HOSTCC)" \
	...
	$$(MESON) \
		...
		--cross-file=$$($$(PKG)_BUILDDIR)/build/cross-compilation.conf \
		-Db_pie=false \
		-Dstrip=false \
		-Dbuild.pkg_config_path=$$(HOST_DIR)/lib/pkgconfig \
		$$($$(PKG)_CONF_OPTS) \
		$$($$(PKG)_SRCDIR) $$($$(PKG)_BUILDDIR)/build
endef
else
...
define $(2)_BUILD_CMDS
	$$(TARGET_MAKE_ENV) $$($$(PKG)_NINJA_ENV) \
		$$(NINJA) $$(NINJA_OPTS) $$($$(PKG)_NINJA_OPTS) -C $$($$(PKG)_BUILDDIR)/build
endef
else
define $(2)_BUILD_CMDS
...
define $(2)_INSTALL_CMDS
	$$(HOST_MAKE_ENV) $$($$(PKG)_NINJA_ENV) \
		$$(NINJA) $$(NINJA_OPTS) -C $$($$(PKG)_BUILDDIR)/build install
endef
endif
