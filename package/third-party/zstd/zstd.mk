...
# zstd supports to configure alternate build directory
ZSTD_SUPPORTS_OUT_SOURCE_BUILD = YES
HOST_ZSTD_SUPPORTS_OUT_SOURCE_BUILD = YES
...
define ZSTD_BUILD_CMDS
	$(TARGET_MAKE_ENV) $(TARGET_CONFIGURE_OPTS) $(MAKE) $(ZSTD_OPTS) \
		-C $(ZSTD_SRCDIR)/lib BUILD_DIR=$(@D)/lib $(ZSTD_BUILD_LIBS) libzstd.pc
	$(TARGET_MAKE_ENV) $(TARGET_CONFIGURE_OPTS) $(MAKE) $(ZSTD_OPTS) \
		-C $(ZSTD_SRCDIR)/programs BUILD_DIR=$(@D)/programs $(ZSTD_BUILD_PROG_TARGET)
endef

define ZSTD_INSTALL_STAGING_CMDS
	$(TARGET_MAKE_ENV) $(TARGET_CONFIGURE_OPTS) $(MAKE) $(ZSTD_OPTS) \
		DESTDIR=$(STAGING_DIR) PREFIX=/usr -C $(ZSTD_SRCDIR)/lib BUILD_DIR=$(@D)/lib \
		install-pc install-includes $(ZSTD_INSTALL_LIBS)
endef

define ZSTD_INSTALL_TARGET_CMDS
	$(TARGET_MAKE_ENV) $(TARGET_CONFIGURE_OPTS) $(MAKE) $(ZSTD_OPTS) \
		DESTDIR=$(TARGET_DIR) -C $(ZSTD_SRCDIR)/programs BUILD_DIR=$(@D)/programs install
	$(TARGET_MAKE_ENV) $(TARGET_CONFIGURE_OPTS) $(MAKE) $(ZSTD_OPTS) \
		DESTDIR=$(TARGET_DIR) -C $(ZSTD_SRCDIR)/lib BUILD_DIR=$(@D)/lib $(ZSTD_INSTALL_LIBS)
endef
...
define HOST_ZSTD_BUILD_CMDS
	$(HOST_MAKE_ENV) $(HOST_CONFIGURE_OPTS) $(MAKE) $(HOST_ZSTD_OPTS) \
		-C $(HOST_ZSTD_SRCDIR) BUILD_DIR=$(@D) zstd-release lib-release
endef

define HOST_ZSTD_INSTALL_CMDS
	$(HOST_MAKE_ENV) $(HOST_CONFIGURE_OPTS) $(MAKE) $(HOST_ZSTD_OPTS) \
		-C $(HOST_ZSTD_SRCDIR) BUILD_DIR=$(@D) install
endef
