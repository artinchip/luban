...
define BUSYBOX_INSTALL_MDEV_SCRIPT
	$(INSTALL) -D -m 0755 package/third-party/busybox/S10mdev \
...
ifeq ($$($(3)_USE_PREBUILT_TARBALL),YES)
	DOTCONFIG=prebuilt/$(BUSYBOX_BASENAME)/target/etc/config/busybox.config
else
	DOTCONFIG=.config
endif
define BUSYBOX_INSTALL_UDHCPC_SCRIPT
	if grep -q CONFIG_UDHCPC=y $(@D)/$(DOTCONFIG); then \
		$(INSTALL) -m 0755 -D package/third-party/busybox/udhcpc.script \
...
define BUSYBOX_INSTALL_ZCIP_SCRIPT
	if grep -q CONFIG_ZCIP=y $(@D)/$(DOTCONFIG); then \
...
define BUSYBOX_SET_GETTY
	$(Q)$(SED) '/# GENERIC_SERIAL$$/s~^.*#~$(SYSTEM_GETTY_PORT)::respawn:/sbin/getty -L $(SYSTEM_GETTY_OPTIONS) $(SYSTEM_GETTY_PORT) $(SYSTEM_GETTY_BAUDRATE) $(SYSTEM_GETTY_TERM) #~' \
		$(TARGET_DIR)/etc/inittab
endef
else
define BUSYBOX_SET_GETTY
	$(Q)$(SED) '/# Put a getty on the serial port$$/s~^.*#~::sysinit:/bin/cat /etc/issue #~' $(TARGET_DIR)/etc/inittab
	$(Q)$(SED) '/# GENERIC_SERIAL$$/s~^.*#~console::respawn:-/bin/sh #~' $(TARGET_DIR)/etc/inittab
endef
...
define BUSYBOX_INSTALL_LOGGING_SCRIPT
	if grep -q CONFIG_SYSLOGD=y $(@D)/$(DOTCONFIG); \
	then \
		$(INSTALL) -m 0755 -D package/third-party/busybox/S01syslogd \
			$(TARGET_DIR)/etc/init.d/S01syslogd; \
	fi; \
	if grep -q CONFIG_KLOGD=y $(@D)/$(DOTCONFIG); \
	then \
		$(INSTALL) -m 0755 -D package/third-party/busybox/S02klogd \
...
define BUSYBOX_INSTALL_SYSCTL_SCRIPT
	if grep -q CONFIG_BB_SYSCTL=y $(@D)/$(DOTCONFIG); \
	then \
		$(INSTALL) -m 0755 -D package/third-party/busybox/S02sysctl \
...
define BUSYBOX_INSTALL_INITTAB
	$(INSTALL) -D -m 0644 package/third-party/busybox/inittab $(TARGET_DIR)/etc/inittab;
endef
endif
...
define BUSYBOX_INSTALL_WATCHDOG_SCRIPT
	$(INSTALL) -D -m 0755 package/third-party/busybox/S15watchdog \
...
define BUSYBOX_INSTALL_TELNET_SCRIPT
	if grep -q CONFIG_FEATURE_TELNETD_STANDALONE=y $(@D)/$(DOTCONFIG); then \
		$(INSTALL) -m 0755 -D package/third-party/busybox/S50telnet \
			$(TARGET_DIR)/etc/init.d/S50telnet ; \
	fi
endef
define BUSYBOX_INSTALL_TELNET_SERVICE
	if grep -q CONFIG_FEATURE_TELNETD_STANDALONE=y $(@D)/$(DOTCONFIG); then \
		$(INSTALL) -D -m 0644 package/third-party/busybox/telnetd.service \
...
define BUSYBOX_INSTALL_ADD_TO_SHELLS
	$(Q)if grep -q CONFIG_ASH=y $(BUSYBOX_DIR)/$(DOTCONFIG); then \
		grep -qsE '^/bin/ash$$' $(TARGET_DIR)/etc/shells \
		|| echo "/bin/ash" >> $(TARGET_DIR)/etc/shells; \
	fi
	$(Q)if grep -q CONFIG_HUSH=y $(BUSYBOX_DIR)/$(DOTCONFIG); then \
...
define BUSYBOX_INSTALL_DOT_CONFIG_TO_TARGET
	$(INSTALL) -m 0644 -D $(@D)/.config $(TARGET_DIR)/etc/config/busybox.config
endef
endif
...
define BUSYBOX_INSTALL_TARGET_CMDS
	...
	$(BUSYBOX_INSTALL_DOT_CONFIG_TO_TARGET)
endef
