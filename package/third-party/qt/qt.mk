...
QT_CONFIGURE_OPTS += -DQT_NO_QWS_CURSOR
QT_CONFIGURE_OPTS += -DQT_NO_CURSOR
...
define QT_INSTALL_STAGING_CMDS
	...
	ln -snf $(STAGING_DIR)/usr/mkspecs $(HOST_DIR)/usr/mkspecs
	$(QT_INSTALL_QT_CONF)
...
define QT_INSTALL_PREBUILT_TARGET_HOOK
	$(Q)if [ -d "$(QT_DIR)/prebuilt/$(QT_BASENAME)/host/" ] ; then \
		rsync -av $(QT_DIR)/prebuilt/$(QT_BASENAME)/host/ $(HOST_DIR); \
	fi
endef

QT_POST_INSTALL_PREBUILT_TARGET_HOOKS += QT_INSTALL_PREBUILT_TARGET_HOOK
