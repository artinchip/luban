...
# Android tools supports to configure alternate build directory
ANDROID_TOOLS_SUPPORTS_OUT_SOURCE_BUILD = YES
HOST_ANDROID_TOOLS_SUPPORTS_OUT_SOURCE_BUILD = YES
...
define HOST_ANDROID_TOOLS_BUILD_CMDS
	$(foreach t,$(HOST_ANDROID_TOOLS_BUILD_TARGETS),\
		mkdir -p $(@D)/build-$(t) && \
		$(HOST_MAKE_ENV) $(HOST_CONFIGURE_OPTS) $(MAKE) SRCDIR=$(HOST_ANDROID_TOOLS_SRCDIR) \
			-C $(@D)/build-$(t) -f $(HOST_ANDROID_TOOLS_SRCDIR)/debian/makefiles/$(t).mk$(sep))
endef

define ANDROID_TOOLS_BUILD_CMDS
	$(foreach t,$(ANDROID_TOOLS_TARGETS),\
		mkdir -p $(@D)/build-$(t) && \
		$(TARGET_MAKE_ENV) $(TARGET_CONFIGURE_OPTS) $(MAKE) SRCDIR=$(ANDROID_TOOLS_SRCDIR) \
			-C $(@D)/build-$(t) -f $(ANDROID_TOOLS_SRCDIR)/debian/makefiles/$(t).mk$(sep))
endef
