		      ...
		      int argc, char *const argv[])
{
	...
	if (argc == 1) {
		...
		if (part > PART_ACCESS_MASK) {
			pr_err("#part_num shouldn't be larger than %d\n",
			       PART_ACCESS_MASK);
			return CMD_RET_FAILURE;
		}
		mmc = init_mmc_device(dev, true);
	...
	pr_info("switch to partitions #%d, %s\n",
	       part, (!ret) ? "OK" : "ERROR");
	if (ret)
		return 1;
	...
	if (mmc->part_config == MMCPART_NOAVAILABLE)
		pr_info("mmc%d is current device\n", curr_device);
	else
		pr_info("mmc%d(part %d) is current device\n",
