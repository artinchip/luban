...
static int fit_add_file_data(struct image_tool_params *params,
		size_t fdt_siz_inc, size_t tmp_siz_inc,
		const char *tmpfile)
{
	int tfd, destfd = 0;
	void *dest_blob = NULL;
	...
	tfd = mmap_fdt(params->cmdname, tmpfile, tmp_siz_inc, &ptr, &sbuf, true,
		       false);
	if (tfd < 0)
		return -EIO;
	...
	if (params->keydest) {
		...
		destfd = mmap_fdt(params->cmdname, params->keydest, fdt_siz_inc,
...
static int fit_handle_file(struct image_tool_params *params)
{
	...
	size_t fdt_siz_inc, tmp_siz_inc;
	int ret;
	 ...
	 */
	for (fdt_siz_inc = 0, tmp_siz_inc = 0; (fdt_siz_inc < 64 * 1024)
			|| (tmp_siz_inc < 64 * 1024);) {
		...
		ret = fit_add_file_data(params, fdt_siz_inc, tmp_siz_inc, tmpfile);
		if (ret == -ENOSPC) {
			fdt_siz_inc += 16;
			tmp_siz_inc += 16;
		} else if (ret == -EFDTNOSPC) {
			fdt_siz_inc += 16;
		} else if (ret == -ETMPNOSPC) {
			tmp_siz_inc += 16;
		}
		if (!ret || (ret != -ENOSPC && ret != -EFDTNOSPC
				&& ret != -ETMPNOSPC))
			break;
	}
