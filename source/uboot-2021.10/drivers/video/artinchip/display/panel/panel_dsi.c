...
{
	static const u8 data[] = { MIPI_DCS_SET_DISPLAY_ON };
	ssize_t err;

	err = mipi_dsi_dcs_send(panel, data, 1);
	if (err < 0)
		return err;
...
{
	static const u8 data[] = { MIPI_DCS_SET_DISPLAY_OFF };
	ssize_t err;
...
{
	static const u8 data[] = { MIPI_DCS_EXIT_SLEEP_MODE };
	ssize_t err;
...
{
	static const u8 data[] = { MIPI_DCS_ENTER_SLEEP_MODE };
	ssize_t err;
...
int panel_dsi_str2fmt(const char *str)
{
	s32 i;
	const char *format[] = {"rgb888", "rgb666l", "rgb666", "rgb565"};

	for (i = 0; i < ARRAY_SIZE(format); i++)
		if (strncasecmp(str, format[i], strlen(format[i])) == 0)
			return i;

	debug("Invalid format: %s", str);
	return DSI_FMT_RGB888;
}

int panel_dsi_str2mode(const char *str)
{
	s32 i;
	const char *mode[] = {"video-pulse", "video-event", "video-burst",
			"command-mode"};
