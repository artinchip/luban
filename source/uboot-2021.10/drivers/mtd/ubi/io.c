		       ...
		       struct ubi_ec_hdr *ec_hdr, int verbose)
{
	...
	crc = ubi_crc32(UBI_CRC32_INIT, ec_hdr, UBI_EC_HDR_SIZE_CRC);
			...
			struct ubi_vid_hdr *vid_hdr, int verbose)
{
	...
	crc = ubi_crc32(UBI_CRC32_INIT, vid_hdr, UBI_VID_HDR_SIZE_CRC);
...
static int self_check_peb_vid_hdr(const struct ubi_device *ubi, int pnum)
{
	...
	crc = ubi_crc32(UBI_CRC32_INIT, vid_hdr, UBI_EC_HDR_SIZE_CRC);
	hdr_crc = be32_to_cpu(vid_hdr->hdr_crc);
	if (hdr_crc != crc) {
