...
/*
 * MTD_UBI_BEB_LIMIT: Maximum expected bad eraseblock count per 1024
 * eraseblocks
 * i: how many blocks in this mtd part
 * (i * CONFIG_MTD_UBI_BEB_LIMIT) / 1024: Maximum expected bad
 * eraseblock count per i eraseblocks
 * mtd->ecc_stats.badblocks: the number of bad blocks identified
 */
static int aic_get_mtd_ubi_beb(struct mtd_info *mtd)
{
#ifdef CONFIG_ARCH_ARTINCHIP
	int mtd_ubi_beb, blkcnt;

	blkcnt = mtd_div_by_eb(mtd->size, mtd);

	mtd_ubi_beb = mtd->ecc_stats.badblocks +
		      (blkcnt * CONFIG_MTD_UBI_BEB_LIMIT) / 1024;

	if (!mtd_ubi_beb)
		mtd_ubi_beb = 1;

	if (mtd->ecc_stats.badblocks)
		pr_info("This mtd part has %d bad blocks\n",
			mtd->ecc_stats.badblocks);

	return mtd_ubi_beb;
#else
	return CONFIG_MTD_UBI_BEB_LIMIT;
#endif
}
		       ...
		       int vid_hdr_offset, int max_beb_per1024)
{
	...
	if (!max_beb_per1024)
		max_beb_per1024 = aic_get_mtd_ubi_beb(mtd);
