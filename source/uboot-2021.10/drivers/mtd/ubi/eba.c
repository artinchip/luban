...
retry:
	...
	if (check) {
		uint32_t crc1 = ubi_crc32(UBI_CRC32_INIT, buf, len);
		if (crc1 != crc) {
			 ...
			 int lnum, const void *buf, int len, int used_ebs)
{
	...
	crc = ubi_crc32(UBI_CRC32_INIT, buf, data_size);
			      ...
			      int lnum, const void *buf, int len)
{
	...
	crc = ubi_crc32(UBI_CRC32_INIT, buf, len);
	vid_hdr->vol_type = UBI_VID_DYNAMIC;
		     ...
		     struct ubi_vid_hdr *vid_hdr)
{
	...
	cond_resched();
	crc = ubi_crc32(UBI_CRC32_INIT, ubi->peb_buf, data_size);
	cond_resched();
	...
	if (data_size > 0) {
		...
		if (crc != ubi_crc32(UBI_CRC32_INIT, ubi->peb_buf, data_size)) {
