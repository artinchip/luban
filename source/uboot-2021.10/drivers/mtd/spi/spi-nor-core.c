...
#ifdef CONFIG_ARTINCHIP_SPIENC
	if (nor->read_opcode == SPINOR_OP_RDSFDP) /* set ciphertext len to 0 */
		spi_mem_enc_xfer_cfg(nor->priv, from, 0, 0);
	else
		spi_mem_enc_xfer_cfg(nor->priv, from, len, 0);
#endif
	while (remaining) {
...
#ifdef CONFIG_ARTINCHIP_SPIENC
		ret = spi_mem_enc_read(nor->priv, &op);
#else
...
#ifdef CONFIG_ARTINCHIP_SPIENC
	spi_mem_enc_xfer_cfg(nor->priv, to, op.data.nbytes, 0);
	ret = spi_mem_enc_write(nor->priv, &op);
#else
	ret = spi_mem_exec_op(nor->spi, &op);
#endif
	if (ret)
...
static int write_cr(struct spi_nor *nor, u8 val)
{
	...
	if (ret < 0) {
		dev_dbg(nor->dev, "error %d writing CR\n", ret);
		return ret;
	}
...
#ifdef CONFIG_ARTINCHIP_SPIENC
		u32 page_size;
		/* Read by page, so that SPIEnc hardware can detect empty
		 * page(all 0xFF) */
		page_size = nor->page_size;
		if (addr & (page_size - 1))
			read_len = page_size - (addr & (page_size - 1));
		else
			read_len = min(nor->page_size, (u32)len);
#endif
...
write_err:
	...
	(defined(CONFIG_SPI_FLASH_MACRONIX) || defined(CONFIG_SPI_FLASH_ISSI))
/**
...
static int spi_nor_sr2_bit1_quad_enable(struct spi_nor *nor)
{
	...
	if (val < 0)
		return val;
	if (val & SR_QUAD_EN_BIT1)
		return 0;

	write_enable(nor);

	ret = write_cr(nor, val | SR_QUAD_EN_BIT1);
	if (ret)
		return ret;
	...
	/* Read back and check it. */
	ret = read_cr(nor);
	if (!(ret > 0 && (ret & SR_QUAD_EN_BIT1))) {
		dev_dbg(nor->dev, "Quad bit not set\n");
		return -EINVAL;
	}
...
static int spi_nor_sr1_bit6_quad_enable(struct spi_nor *nor)
{
	...
	if (!(ret > 0 && (ret & SR_QUAD_EN_BIT6))) {
		dev_err(nor->dev, "Quad bit not set\n");
		return -EINVAL;
	}
	...
	if (info->flags & SPI_NOR_QUAD_READ) {
		...
		case SNOR_MFR_MICRON:
			...
			spi_nor_set_pp_settings(&params->page_programs[SNOR_CMD_PP_1_1_4],
					SPINOR_MX_OP_PP_1_1_4, SNOR_PROTO_1_1_4);
			break;
		case SNOR_MFR_ATMEL:
			params->hwcaps.mask |= SNOR_HWCAPS_PP_1_1_4;
			spi_nor_set_pp_settings(&params->page_programs[SNOR_CMD_PP_1_1_4],						SPINOR_AT_OP_PP_1_1_4, SNOR_PROTO_1_1_4);
			break;
		default:
			params->hwcaps.mask |= SNOR_HWCAPS_PP_1_1_4;
			spi_nor_set_pp_settings(&params->page_programs[SNOR_CMD_PP_1_1_4],
