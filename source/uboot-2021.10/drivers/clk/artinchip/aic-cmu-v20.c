...
/* clk-pll_fra */
#define CLK_PLL_FRA_GEN_REG(x)	((x) * 0x4 + 0x0000)
#define CLK_PLL_FRA_CFG_REG(x)	((x) * 0x4 + 0x0030)
#define CLK_PLL_FRA_SDM_REG(x)	((x) * 0x4 + 0x0060)
...
#define CLK_GPT_SDFM_REG	(0x024C)
...
static struct aic_pll clk_plls[] = {
	...
	CLK_PLL(CLK_PLL_FRA8, CLK_PLL_FRA_GEN_REG(8), CLK_PLL_FRA_CFG_REG(8),
		CLK_PLL_FRA_SDM_REG(8), AIC_PLL_FRA),
};
...
static struct aic_sys_clk clk_syss[] = {
	...
	CLK_SYS_BUS(CLK_APB0, CLK_APB0_REG, bus_src_sels,
		    ARRAY_SIZE(bus_src_sels), 8, 1, 0, 4),
...
static struct aic_periph_clk clk_periphs[] = {
	/*              id               parent           reg           */
	CLK_PERIPH(CLK_DMA0, CLK_24M, CLK_DMA0_REG, 12, -1, 0, 0),
	CLK_PERIPH(CLK_SE_CE, CLK_PLL_FRA1, CLK_SE_CE_REG, 12, -1, 0, 0),
	CLK_PERIPH(CLK_SYSCFG, CLK_24M, CLK_SYSCFG_REG, 12, -1, 0, 0),
#if !defined(CONFIG_SPL_BUILD)
	CLK_PERIPH(CLK_USBH0, CLK_24M, CLK_USBH0_REG, 12, -1, 0, 0),
	CLK_PERIPH(CLK_USBH1, CLK_24M, CLK_USBH1_REG, 12, -1, 0, 0),
	CLK_PERIPH(CLK_USB_PHY0, CLK_24M, CLK_USB_PHY0_REG, -1, 8, 0, 0),
	CLK_PERIPH(CLK_USB_PHY1, CLK_24M, CLK_USB_PHY1_REG, -1, 8, 0, 0),
#endif
...
#endif
	CLK_PERIPH(CLK_SDMC0, CLK_PLL_FRA2, CLK_SDMC0_REG, 12, 8, 0, 4),
	CLK_PERIPH(CLK_SDMC1, CLK_PLL_FRA2, CLK_SDMC1_REG, 12, 8, 0, 4),
#if !defined(CONFIG_SPL_BUILD)
	...
	CLK_PERIPH(CLK_ADCIM, CLK_PLL_FRA1, CLK_ADCIM_REG, 12, 8, 0, 5),
...
static struct aic_disp_clk clk_disps[] = {
	CLK_DISP(CLK_SCLK, CLK_PLL_FRA4, CLK_DISP_REG, 0, 3, 0, 0, 0, 0, 0, 0),
	CLK_DISP(CLK_PIX, CLK_SCLK, CLK_DISP_REG, 0, 0, 4, 5, 10, 2, 0, 0),
};
...
static struct aic_clk_crosszone clk_crosszone[] = {
	CLK_PERIPH(CLK_SP_USBD, CLK_24M, CLK_SP_USBD_REG, 12, 0, 0, 0),
	CLK_PERIPH(CLK_SP_USB_PHY0, CLK_24M, CLK_SP_USB_PHY0_REG, 0, 8, 0, 0),
};
...
struct aic_clk_tree aic_clktree = {
	...
	.cross_zone_base = CLK_SP_USBD,
	.cross_zone_cnt = ARRAY_SIZE(clk_crosszone),
...
static void aic_pll_init_platdata(struct udevice *dev)
{
	...
	struct dtd_artinchip_aic_cmu_v2_0 *dtplat = &plat->dtplat;
	...
	clk.id = CLK_PLL_FRA7;
	clk_set_rate(&clk, dtplat->pll_frac7);

	clk.id = CLK_PLL_FRA8;
	clk_set_rate(&clk, dtplat->pll_frac8);
}
#else
static void aic_pll_init_ofdata(struct udevice *dev)
{
	...
	ret = dev_read_u32(dev, "pll-frac8", &value);
	if (!ret) {
		clk.id = CLK_PLL_FRA8;
		clk.dev = dev;
		freq = clk_get_rate(&clk);
...
static const struct udevice_id aic_clk_ids[] = {
	{ .compatible = "artinchip,aic-cmu-v2.0", },
	{ }
};

U_BOOT_DRIVER(artinchip_cmu) = {
	.name                     = "artinchip_aic_cmu_v2_0",
	.id                       = UCLASS_CLK,
