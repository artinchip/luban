				    ...
				    uint64_t size, enum lmb_flags flags)
{
	...
	if (ret >= 0) {
		...
		pr_info("(addr=%llx size=%llx flags=%x)\n",
		       (unsigned long long)addr,
		       (unsigned long long)size, flags);
	}
}
...
int boot_relocate_fdt(struct lmb *lmb, char **of_flat_tree, ulong *of_size)
{
	...
	if (disable_relocation) {
		...
		pr_info("   Using Device Tree in place at %p, end %p\n",
	...
	*of_size = 0;
	...
	if (select || genimg_has_config(images)) {
		...
		case IMAGE_FORMAT_LEGACY:
			...
			pr_info("## Flattened Device Tree from Legacy Image at %08lx\n",
			       fdt_addr);
		 ...
		 */
		pr_info("## Flattened Device Tree from multi component Image at %08lX\n",
		...
		if (fdt_len) {
			...
			pr_info("   Booting using the fdt at 0x%p\n", fdt_blob);
		       ...
		       int of_size, struct lmb *lmb)
{
	...
	if (fdt_root(blob) < 0) {
		pr_info("ERROR: root node setup failed\n");
		goto err;
	}
	if (fdt_chosen(blob) < 0) {
		pr_info("ERROR: /chosen node create failed\n");
		goto err;
	}
	if (arch_fixup_fdt(blob) < 0) {
		pr_info("ERROR: arch-specific fdt fixup failed\n");
		goto err;
	}

	fdt_ret = optee_copy_fdt_nodes(blob);
	if (fdt_ret) {
		pr_info("ERROR: transfer of optee nodes to new fdt failed: %s\n",
	...
	if (IMAGE_OF_BOARD_SETUP) {
		...
		if (skip_board_fixup && ((int)simple_strtol(skip_board_fixup, NULL, 10) == 1)) {
			pr_info("skip board fdt fixup\n");
		} else {
			fdt_ret = ft_board_setup(blob, gd->bd);
			if (fdt_ret) {
				pr_info("ERROR: board-specific fdt fixup failed: %s\n",
				       fdt_strerror(fdt_ret));
				goto err;
			}
		}
	...
	if (IMAGE_OF_SYSTEM_SETUP) {
		...
		if (fdt_ret) {
			pr_info("ERROR: system-specific fdt fixup failed: %s\n",
			       fdt_strerror(fdt_ret));
			goto err;
		}
	}
	...
	return 0;
err:
	pr_info(" - must RESET the board to recover.\n\n");
