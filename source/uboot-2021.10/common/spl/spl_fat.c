		       ...
		       const char *filename)
{
	...
	loff_t actread;
	ulong offset = 0, maxsize = sizeof(struct image_header);
#ifdef CONFIG_UPDATE_ARTINCHIP_BOOTCFG
...
#ifdef CONFIG_UPDATE_ARTINCHIP_BOOTCFG
	err = fat_read_file(filename, header, 0, 1024, &actread);
	if (err)
		goto end;
	err = boot_cfg_get_boot1((char *)header, actread, imgname,
				 IMG_NAME_MAX_SIZ, &offset, &maxsize);
	if (err <= 0)
		goto end;
	/* Point to real image data file */
	filename = imgname;
#endif
	err = fat_read_file(filename, header, offset,
	...
	if (IS_ENABLED(CONFIG_SPL_LOAD_FIT_FULL) &&
		...
		err = fat_read_file(filename, (void *)CONFIG_SYS_LOAD_ADDR,
		...
		err = fat_read_file(filename,
				    (u8 *)(uintptr_t)spl_image->load_addr,
				    offset, maxsize, &actread);
	}
...
#if defined(CONFIG_SPL_OS_BOOT) && !defined(CONFIG_ARCH_ARTINCHIP)
