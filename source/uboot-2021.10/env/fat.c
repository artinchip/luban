...
#ifdef CONFIG_UPDATE_ARTINCHIP_BOOTCFG
	loff_t actread;
	ulong offset = 0, maxsize = 0;
	char imgname[IMG_NAME_MAX_SIZ];

	/* Read the bootcfg.txt */
	err1 = fat_read_file(CONFIG_ENV_FAT_FILE, buf1, 0, 0, &actread);
	if (err1 < 0)
		return -EIO;
	err1 = boot_cfg_get_env(buf1, actread, imgname, IMG_NAME_MAX_SIZ,
			       &offset, &maxsize);
	if (err1 < 0)
		return -EFAULT;
	err1 = fat_read_file(imgname, buf1, offset, maxsize, &actread);
#else
