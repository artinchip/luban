...
static struct env_driver *env_driver_lookup(enum env_operation op, int prio)
{
	...
	if (!drv) {
		log_warning("%s: No environment driver for location %d\n",
			    __func__, loc);
		return NULL;
	}
...
int env_load(void)
{
	...
	for (prio = 0; (drv = env_driver_lookup(ENVOP_LOAD, prio)); prio++) {
		...
		log_notice("Loading Environment from %s... ", drv->name);
		/*
	 ...
	 */
	if (best_prio >= 0)
		log_warning("Selecting environment with bad CRC\n");
	else
		best_prio = 0;
...
int env_init(void)
{
	...
	for (prio = 0; (drv = env_driver_lookup(ENVOP_INIT, prio)); prio++) {
		...
		log_notice("%s: Environment %s init done (ret=%d)\n", __func__,
			   drv->name, ret);
