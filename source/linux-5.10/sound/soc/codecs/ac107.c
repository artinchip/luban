...
#if AC107_DEBUG_EN
#define AC107_DEBUG(...)		pr_info(__VA_ARGS__)
#else
#define AC107_DEBUG(...)
#endif

/* 0:ADC normal,  1:0x5A5A5A,  2:0x123456,
 * 3:0x000000,  4~7:I2S_RX_DATA,  other:reserved
 */
#define AC107_ADC_PATTERN_SEL	ADC_PTN_NORMAL

/* AC107 config */
#define AC107_CHIP_NUMS			1	/* range[1, 8] */
#define AC107_CHIP_NUMS_MAX		8	/* range[1, 8] */
/* 8/12/16/20/24/28/32bit Slot Width */
#define AC107_SLOT_WIDTH		16
#define AC107_ENCODING_EN		0	/* TX Encoding mode enable */
/* TX Encoding channel numbers, must be dual, range[1, 16] */
#define AC107_ENCODING_CH_NUMS	2
/* TX Encoding format: 0:first channel number 0, other:first channel number 1 */
#define AC107_ENCODING_FMT		0
/*range[1, 1024], default PCM mode, I2S/LJ/RJ mode shall divide by 2
 *#define AC107_LRCK_PERIOD
 *(AC107_SLOT_WIDTH*(AC107_ENCODING_EN ? 2 : AC107_CHIP_NUMS*2))
 */
#define AC107_LRCK_PERIOD \
		(AC107_SLOT_WIDTH * (AC107_ENCODING_EN ? 2 : AC107_CHIP_NUMS))
/* AC107 match method select: 0: i2c_detect, 1:devices tree */
#define AC107_MATCH_DTS_EN		1

#define AC107_KCONTROL_EN		1
#define AC107_DAPM_EN			0
#define AC107_CODEC_RW_USER_EN	1
//-6dB and 0dB, 3~30dB, 1dB step
#define AC107_PGA_GAIN			ADC_PGA_GAIN_28dB
...
int ac107_codec##n##_get(struct snd_kcontrol *kcontrol,\
...
static const DECLARE_TLV_DB_SCALE(digital_mix_vol_tlv,
								-600, 600, 0);
...
static const struct snd_kcontrol_new ac107_controls[] = {
	...
	/*SOC_SINGLE("SYSCLK Hold Time Sel",\
	 *PLL_LOCK_CTRL, SYSCLK_HOLD_TIME, 0x7, 0),
	 */
};

/************************** DAPM controls ************************************/
//ADC DMIC Source Select MUX
static const char * const adc_dmic_src_mux_text[] = {
...
static const char * const i2s_tx_ch1_map_mux_text[] = {
...
static const char * const i2s_tx_ch2_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static const char * const i2s_tx_ch3_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static const char * const i2s_tx_ch4_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static const char * const i2s_tx_ch5_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static const char * const i2s_tx_ch6_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static const char * const i2s_tx_ch7_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static const char * const i2s_tx_ch8_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static const char * const i2s_tx_ch9_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static const char * const i2s_tx_ch10_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static const char * const i2s_tx_ch11_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static const char * const i2s_tx_ch12_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static const char * const i2s_tx_ch13_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static const char * const i2s_tx_ch14_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static const char * const i2s_tx_ch15_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static const char * const i2s_tx_ch16_map_mux_text[] = {
	"ADC1 Sample switch", "ADC2 Sample switch"
};
...
static int ac107_read(u8 reg, u8 *rt_value, struct i2c_client *client)
{
	...
	if (!client || !client->adapter) {
		pr_err("%s client or client->adapter is NULL\n", __func__);
		return -1;
	}

	ret = i2c_master_send(client, read_cmd, cmd_len);
	if (ret != cmd_len) {
		pr_err("%s error1->[REG-0x%02x]\n", __func__, reg);
		return -1;
	}

	ret = i2c_master_recv(client, rt_value, 1);
	if (ret != 1) {
		pr_err("%s error2->[REG-0x%02x], ret=%d\n", __func__, reg, ret);
		return -1;
	}
...
static int ac107_write(u8 reg, unsigned char value,
				struct i2c_client *client)
{
	int ret = 0;
	u8 write_cmd[2] = { 0 };
...
static void ac107_hw_init(struct i2c_client *i2c)
{
	...
	/*** SYSCLK Config ***/
	/*SYSCLK Enable */
	ac107_update_bits(SYSCLK_CTRL, 0x1 << SYSCLK_EN, 0x1 << SYSCLK_EN, i2c);
	/*0x21=0x07: Module clock enable<I2S, ADC digital,  ADC analog> */
	ac107_write(MOD_CLK_EN, 0x07, i2c);
	/*0x22=0x03: Module reset de-asserted<I2S, ADC digital> */
	ac107_write(MOD_RST_CTRL, 0x03, i2c);

	/*** I2S Common Config ***/
	/*SDO enable */
	ac107_update_bits(I2S_CTRL, 0x1 << SDO_EN, 0x1 << SDO_EN, i2c);
	/*SDO drive data and SDI sample data at the different BCLK edge */
	ac107_update_bits(I2S_BCLK_CTRL, 0x1 << EDGE_TRANSFER,
				0x0 << EDGE_TRANSFER, i2c);
	ac107_update_bits(I2S_LRCK_CTRL1, 0x3 << LRCK_PERIODH,
				((AC107_LRCK_PERIOD - 1) >> 8) << LRCK_PERIODH,
				i2c);
	/*config LRCK period */
	ac107_write(I2S_LRCK_CTRL2, (u8) (AC107_LRCK_PERIOD - 1), i2c);
	/*Encoding mode format select 0~N-1, Encoding mode enable,
	...
	ac107_update_bits(I2S_FMT_CTRL1,
			...
			| 0x1 << TX_STATE, i2c);
	/*8/12/16/20/24/28/32bit Slot Width */
	ac107_update_bits(I2S_FMT_CTRL2, 0x7 << SLOT_WIDTH_SEL,
			(AC107_SLOT_WIDTH / 4 - 1) << SLOT_WIDTH_SEL, i2c);
	/*0x36=0x60: TX MSB first, SDOUT normal, PCM frame type,
	 *Linear PCM Data Mode
	 */
	ac107_update_bits(I2S_FMT_CTRL3,
			...
			| 0x0 << TX_PDM, i2c);
	/*0x3c=0xaa: TX CH1/3/5/7 map to adc1, TX CH2/4/6/8 map to adc2 */
	ac107_update_bits(I2S_TX_CHMP_CTRL1, !AC107_DAPM_EN * 0xff, 0xaa, i2c);
	//0x3d=0xaa: TX CH9/11/13/15 map to adc1, TX CH10/12/14/16 map to adc2
	ac107_update_bits(I2S_TX_CHMP_CTRL2, !AC107_DAPM_EN * 0xff, 0xaa, i2c);

	/*PDM Interface Latch ADC1 data on rising clock edge.
	 *Latch ADC2 data on falling clock edge, PDM Enable
	 */
	ac107_update_bits(PDM_CTRL, 0x1 << PDM_TIMING | 0x1 << PDM_EN,
			  0x0 << PDM_TIMING | !!AC107_PDM_EN << PDM_EN, i2c);

	/*** ADC DIG part Config***/
	/*0x61=0x07: Digital part globe enable, ADCs digital part enable */
	ac107_update_bits(ADC_DIG_EN, !AC107_DAPM_EN * 0x7, 0x7, i2c);
	/*DMIC Enable */
	ac107_update_bits(DMIC_EN, !AC107_DAPM_EN * 0x1, !!AC107_DMIC_EN, i2c);
			    ...
			    unsigned int freq, int dir)
{
	...
	case SYSCLK_SRC_MCLK:
		...
		ac107_multi_chips_update_bits(SYSCLK_CTRL, 0x3 << SYSCLK_SRC,
			SYSCLK_SRC_MCLK << SYSCLK_SRC);
		break;
	case SYSCLK_SRC_BCLK:
		AC107_DEBUG("AC107 SYSCLK source select BCLK\n\n");
		//System Clock Source Select BCLK
		ac107_multi_chips_update_bits(SYSCLK_CTRL, 0x3 << SYSCLK_SRC,
			SYSCLK_SRC_BCLK << SYSCLK_SRC);
		break;
	case SYSCLK_SRC_PLL:
			 ...
			 unsigned int freq_in, unsigned int freq_out)
{
	...
	if (freq_in < 128000 || freq_in > 24576000) {
		pr_err
		    ("AC107 PLLCLK source input freq only support"
		     " [128K,24M],while now %u\n\n",
		     freq_in);
		return -EINVAL;
	...
	//PLL module enable
	//PLL CLK lock enable
	ac107_multi_chips_update_bits(PLL_LOCK_CTRL, 0x1 << PLL_LOCK_EN,
					0x1 << PLL_LOCK_EN);
	//PLL Common voltage Enable, PLL Enable
	/*ac107_multi_chips_update_bits(PLL_CTRL1,\
	 *0x1<<PLL_EN | 0x1<<PLL_COM_EN, 0x1<<PLL_EN | 0x1<<PLL_COM_EN);
	 */
...
static int ac107_set_clkdiv(struct snd_soc_dai *dai, int div_id, int div)
{
	...
	if (!div_id) {
		AC107_DEBUG
		    ("AC107 work as Slave mode, "
		     "don't need to config BCLK_DIV\n\n");
		return 0;
	}
...
static int ac107_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBM_CFM:	//AC107 Master
		...
		//BCLK & LRCK output
		ac107_update_bits(I2S_CTRL, 0x3 << LRCK_IOEN,
					0x3 << LRCK_IOEN, ac107->i2c);
		break;
	case SND_SOC_DAIFMT_CBS_CFS:	//AC107 Slave
		AC107_DEBUG("AC107 set to work as Slave\n");
		//BCLK & LRCK input
		ac107_update_bits(I2S_CTRL, 0x3 << LRCK_IOEN,
					0x0 << LRCK_IOEN, ac107->i2c);
		break;
	default:
			   ...
			   struct snd_soc_dai *dai)
{
	...
	if (i & 0x80) {		//PLLCLK Enable
		if (!(i & 0x0c)) {	//SYSCLK select MCLK
			...
			/*multi_chips: only one chip MCLK output PLL_test,
			 *and the others MCLK config as input
			 */
			for (i = 0; i < AC107_CHIP_NUMS; i++) {
				if (i2c_ctrl[i] == ac107->i2c)
					continue;
			...
			/*the chip which MCLK config as output,
			 *should select PLL as its SYCCLK source
			 */
...
static int ac107_probe(struct snd_soc_component *component)
{
	...
	ret = IS_ERR(component->regmap);
	if (ret) {
		dev_err(component->dev,
			"AC107 regmap init I2C Failed: %d\n", ret);
		return ret;
	}
	ac107->component = component;

#if AC107_KCONTROL_EN
	//ADC1 PGA Gain default
	ac107_multi_chips_update_bits(ANA_ADC1_CTRL3, 0x1f << RX1_PGA_GAIN_CTRL,
			AC107_PGA_GAIN << RX1_PGA_GAIN_CTRL);
	//ADC2 PGA Gain default
	ac107_multi_chips_update_bits(ANA_ADC2_CTRL3, 0x1f << RX2_PGA_GAIN_CTRL,
			AC107_PGA_GAIN << RX2_PGA_GAIN_CTRL);
	//PGA Gain Control
	snd_soc_add_component_controls(component, ac107_volume_controls,
			AC107_CHIP_NUMS * 2);
	//Digital Volume Control
	snd_soc_add_component_controls(component, ac107_volume_controls + 16,
			AC107_CHIP_NUMS * 2);
#endif

#if AC107_DAPM_EN
	//defatul map:TX CH1/3/5/7 map to adc1, TX CH2/4/6/8 map to adc2
	ac107_multi_chips_write(I2S_TX_CHMP_CTRL1, 0xaa);
	//defatul map:TX CH9/11/13/15 map to adc1, TX CH10/12/14/16 map to adc2
	ac107_multi_chips_write(I2S_TX_CHMP_CTRL2, 0xaa);
#endif
...
static int ac107_resume(struct snd_soc_component *component)
{
	...
	if ((ac107_regulator_en & 0x1) && !IS_ERR(ac107->vol_supply.dvcc_1v8)) {
		...
		if (ret != 0)
			pr_err
			    ("[AC107] %s: some error happen, "
			     "fail to enable regulator dvcc_1v8!\n",
			     __func__);
		ac107_regulator_en |= 0x1;
	}
	...
	if ((ac107_regulator_en & 0x2)
		...
		if (ret != 0)
			pr_err
			    ("[AC107] %s: some error happen, "
			     "fail to enable regulator avcc_vccio_3v3!\n",
			     __func__);
		ac107_regulator_en |= 0x2;
	}
	if (gpio_is_valid(ac107->reset_gpio)) {
...
static unsigned int ac107_component_read(
...
static int ac107_component_write(struct snd_soc_component *component,
			   ...
			   const char *buf, size_t count)
{
	...
	if (flag) {
		reg = (val >> 8) & 0xFF;
		value_w = val & 0xFF;
		pr_info("\nWrite: start REG:0x%02x,val:0x%02x,"
					"count:0x%02x\n",
		       reg, value_w, flag);
		while (flag--) {
...
static ssize_t ac107_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	pr_info("/*** AC107 driver version: V1.0 ***/\n");
	pr_info("echo flag|reg|val > ac107\n");
	pr_info("eg->read start address=0x00,count=0xff:"
			   "echo 00ff >ac107\n");
	pr_info("eg->write start address=0x70,"
	     "value=0xa0,count=0x2: echo 270a0 >ac107\n");
	//printk("eg write value:0xfe to address:0x06 :echo 106fe > ac107\n");
	return 0;
}

static DEVICE_ATTR_RW(ac107);
			   ...
			   const struct i2c_device_id *i2c_id)
{
	...
	if (!ac107_regulator_en) {
		//(const char**)
		ret = of_property_read_string(np, AC107_DVCC_NAME,
				&regulator_name);
		if (ret)
		...
		//(const char**)
		ret = of_property_read_string(np, AC107_AVCC_VCCIO_NAME,
				&regulator_name);
		if (ret)
...
static const struct of_device_id ac107_dt_ids[] = {
#if AC107_CHIP_NUMS > 0
	{.compatible = "ac107_0",},
#endif
