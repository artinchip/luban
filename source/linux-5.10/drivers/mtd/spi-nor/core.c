			...
			size_t *retlen, u_char *buf)
{
	...
	while (len) {
		 ...
		 * page(all 0xFF) */
		if (addr & (nor->page_size - 1))
			ret = nor->page_size - (addr & (nor->page_size - 1));
		else
			ret = min((ssize_t)nor->page_size, (ssize_t)len);
...
#ifdef CONFIG_CRYPTO_DEV_ARTINCHIP_SPIENC
		ret = spi_nor_enc_write(nor, addr, page_remain, buf + i);
#else
