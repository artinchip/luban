...
static u64 ov5640_calc_pixel_rate(struct ov5640_dev *sensor)
{
	...
	dev_info(&sensor->i2c_client->dev, "Pixel rate: %ld MHz, %d fps\n",
		 (long)rate / 1000000, ov5640_framerates[sensor->current_fr]);
	return rate;
}
...
{
	const struct ov5640_mode_info *mode;
	enum ov5640_frame_rate rate = OV5640_10_FPS;
	int minfps, maxfps, best_fps, fps;
	int i;

	minfps = ov5640_framerates[OV5640_10_FPS];
...
static int ov5640_probe(struct i2c_client *client)
{
	...
	ret = of_property_read_u32(dev->of_node, "clock-rate",
				   &sensor->xclk_freq);
	if (!ret)
