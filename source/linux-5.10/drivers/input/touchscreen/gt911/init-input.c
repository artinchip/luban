 ...
 *               -EIO :  i/o err.
 */
int input_set_int_enable(enum input_sensor_type *ctp_type, u32 enable)
{
	int ret = -1;
	u32 irq_number = 0;
	...
	{
		case CTP_TYPE:
			data = container_of(ctp_type, struct ctp_config_info, input_type);
 ...
 *               -EIO :  i/o err.
 */
int input_free_int(enum input_sensor_type *ctp_type, void *para)
{
	int irq_number = 0;
	struct ctp_config_info *data = NULL;
 ...
 *
 */
int input_request_int(enum input_sensor_type *ctp_type, irq_handler_t handle,
