...
#define CTP_NAME		("gt911")
#define CTP_IRQ_MODE			IRQF_TRIGGER_RISING //(IRQF_TRIGGER_LOW)//(IRQF_TRIGGER_FALLING)
...
static u8 g_IntFlag = 0, g_IntCnt = 0;
...
static const struct file_operations file_config_proc_ops = {
    .owner = THIS_MODULE,
...
static void __iomem*gpio_base_virtaddr = NULL;
...
static int ctp_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	printk("gt9xx ctp_detect config_info.twi_id = %d, adapter->nr = %d\n", config_info.twi_id, adapter->nr);
	...
	{
		printk("gt9xx ctp_detect twi_id error\n");
		return -ENODEV;
	}
}
...
void gtp_set_gpio_output(int gpionum,int value)
{
	u32 regvalue; 
	u32 bit = gpionum%8;
	
	regvalue = readl(gpio_base_virtaddr+0x4); 
	regvalue |= (1 << bit); 
	writel(regvalue,gpio_base_virtaddr+0x4);
	regvalue = readl(gpio_base_virtaddr+0x8); 
	regvalue |= (1 << bit); 
	writel(regvalue,gpio_base_virtaddr+0x8);

	regvalue = readl(gpio_base_virtaddr); 
	regvalue &= ~(0x1 <<bit); 
	regvalue |= (value << bit); 
	writel(regvalue,gpio_base_virtaddr);
	
}

void gtp_set_int_value(int status)
{
	gtp_set_gpio_output(config_info.irq_gpio, status);
    //gpio_set_value(config_info.irq_gpio, status);
}

void gtp_set_io_int(void)
{
	gpio_direction_input(config_info.irq_gpio);
}
...
{      
	...
	ctp_wakeup(1, 0);
	msleep(6);
	gtp_set_gpio_output(config_info.wakeup_gpio, 1);
	gtp_int_sync(50);
}
...
void gtp_irq_disable(struct goodix_ts_data *ts)
{
    ...
    {
        ts->irq_is_disable = 1; 
		if(input_set_int_enable(&(config_info.input_type), 0) < 0)
...
void gtp_irq_enable(struct goodix_ts_data *ts)
{
    ...
    {
        ts->irq_is_disable = 0; 
		if(input_set_int_enable(&(config_info.input_type), 1) < 0)	            
...
{
    GTP_DEBUG("ID0:%d, X0:%d, Y0:%d, W0:%d", id, x, y, w);
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
  ...
  struct goodix_ts_data *ts = i2c_get_clientdata(client);
	...
	printk("gtp_reset_guitar\n");

	gpio_set_value(config_info.wakeup_gpio, 0);	// begin select I2C slave addr
    msleep(ms);                         // T2: > 10ms
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    u8 cfg_info_group6[] = CTP_CFG_GROUP6;
      u8 *send_cfg_buf[] = {gtp_dat_7, 
							cfg_info_group2, cfg_info_group3,
                            cfg_info_group4, cfg_info_group5, cfg_info_group6};
    u8 cfg_info_len[] = {CFG_GROUP_LEN(gtp_dat_7),
    ...
    else
    {
        ...
        if (SUCCESS == ret)
        {
			...
			GTP_INFO("GTP_REG_SENSOR_ID%d , config length: %d", sensor_id, ts->gtp_cfg_len);
            if (sensor_id >= 0x06)
            {
...
#if GTP_ICS_SLOT_REPORT
    input_mt_init_slots(ts->input_dev, 5);     // in case of "out of memory"
#else
...
//    input_set_abs_params(ts->input_dev, ABS_MT_TRACKING_ID, 0, 255, 0, 0);
	...
	input_set_abs_params(ts->input_dev, ABS_X, 0, config_info.screen_max_x, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_Y, 0, config_info.screen_max_y, 0, 0);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //ts->abs_x_max = config_info.screen_max_x;
    //ts->abs_y_max = config_info.screen_max_y;
...
#if GTP_ESD_PROTECT
    ts->clk_tick_cnt = 5 * HZ;      // HZ: clock ticks in 5 second generated by system
	...
	printk("goodix_ts_probe success !!! \n");
    return 0;
...
static int goodix_ts_init(void)
{
	...
	{		
		GTP_ERROR("%s: ctp_fetch_sysconfig_para err.\n", __func__);		
		return -1;	
	}	
	else	
	{		
		...
		if (0 != ret)		
		{		
			GTP_ERROR("%s:ctp_ops.init_platform_resource err. \n", __func__);
			return ret;
		}	
	}	
	...
	{	   
		GTP_ERROR("*** ctp_used set to 0 !\n"); 	
		GTP_ERROR("*** if use ctp,please put the sys_config.fex ctp_used set to 1. \n");	
		return 0;	
	}	

	phyaddr += 0x10000 *(config_info.irq_gpio/8); 
	//gpio_base_virtaddr = ioremap_nocache(phyaddr, 0x200);
	gpio_base_virtaddr = ioremap(phyaddr, 0x200);
	if(gpio_base_virtaddr == NULL)
	{
		GTP_ERROR("*** ctp ioremap_nocache gpio %x fail !\n",phyaddr); 
		return 0;
	}
...
static void goodix_ts_exit(void)
{
	...
	if(gpio_base_virtaddr)
		iounmap(gpio_base_virtaddr);
	gpio_base_virtaddr = NULL;
}
