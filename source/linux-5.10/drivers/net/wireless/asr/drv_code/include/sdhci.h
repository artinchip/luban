...
#define SDHCI_QUIRK2_NO_CURRENT_LIMIT                   (1<<4)
/* Controller data timeout counter is 4 times long as spec defined */
#define SDHCI_QUIRK2_TIMEOUT_DIVIDE_4			(1<<5)
...
#define SDHCI_QUIRK2_TUNING_SW_BROKEN			(1<<11)
/* Controller supported max data timeout is too short (~10s for 52Mhz bus clock) */
#define SDHCI_QUIRK2_TIMEOUT_SHORT			(1<<12)
/* some SD host need to set IO capability by SOC part register */
#define SDHCI_QUIRK2_SET_AIB_MMC			(1<<13)
/* Base clock must always on during working. Some sdio card and sd card host detect need this */
#define SDHCI_QUIRK2_BASE_CLOCK_ALWAYS_ON		(1<<14)
/* Card detect will be triggered manually */
#define SDHCI_QUIRK2_MANUAL_CARD_DETECT			(1<<15)
/* HS200/SDR104 tuning broken and must use predefined fixed delay */
#define SDHCI_QUIRK2_TUNING_BROKEN			(1<<16)
/* Vqmmc needs always on to keep host power on for card detection */
#define SDHCI_QUIRK2_VQMMC_ALWAYS_ON			(1<<17)
/* Fake cmd index error */
#define SDHCI_QUIRK2_FAKE_CMD_INDEX_ERR			(1<<18)
/* DMA need internal clock force on */
#define SDHCI_QUIRK2_DMA_CLOCK_FORCE_ON			(1<<19)
/* Disable Timer re-tuning */
#define SDHCI_QUIRK2_NO_TIMER_RETUNING			(1<<20)
/* Some platform may use ASYNC_INT for clock gating */
#define SDHCI_QUIRK2_CLK_GATE_USE_ASYNC_INT		(1<<21)
	int irq;		/* Device IRQ */
...
#define	MAX_ADMA_DESC	3
	...
	dma_addr_t adma_addr[MAX_ADMA_DESC];	/* Mapped ADMA descr. table */
	dma_addr_t align_addr[MAX_ADMA_DESC];	/* Mapped bounce buffer */
