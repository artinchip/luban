 ...
 */
#undef TRACE_SYSTEM
#define TRACE_SYSTEM asr

#if !defined(_ASR_EVENTS_H) || defined(TRACE_HEADER_MULTI_READ)
#define _ASR_EVENTS_H
...
TRACE_EVENT(roc,
	    ...
	    TP_ARGS(vif_idx, freq, duration), TP_STRUCT__entry(__field(u8, vif_idx)
							       __field(u16, freq)
							       __field(unsigned int, duration)
	    ), TP_fast_assign(__entry->vif_idx = vif_idx;
			      __entry->freq = freq; __entry->duration = duration;
	    ), TP_printk("f=%d vif=%d dur=%d", __entry->freq, __entry->vif_idx, __entry->duration)
    );

TRACE_EVENT(cancel_roc, TP_PROTO(u8 vif_idx), TP_ARGS(vif_idx), TP_STRUCT__entry(__field(u8, vif_idx)
	    ), TP_fast_assign(__entry->vif_idx = vif_idx;
	    ), TP_printk("vif=%d", __entry->vif_idx)
    );

TRACE_EVENT(roc_exp, TP_PROTO(u8 vif_idx), TP_ARGS(vif_idx), TP_STRUCT__entry(__field(u8, vif_idx)
	    ), TP_fast_assign(__entry->vif_idx = vif_idx;
	    ), TP_printk("vif=%d", __entry->vif_idx)
    );

TRACE_EVENT(switch_roc, TP_PROTO(u8 vif_idx), TP_ARGS(vif_idx), TP_STRUCT__entry(__field(u8, vif_idx)
	    ), TP_fast_assign(__entry->vif_idx = vif_idx;
	    ), TP_printk("vif=%d", __entry->vif_idx)
    );
...
DECLARE_EVENT_CLASS(mgmt_template,
		    TP_PROTO(u16 freq, u8 vif_idx, u8 sta_idx,
			     struct ieee80211_mgmt *mgmt), TP_ARGS(freq,
								   vif_idx,
								   sta_idx,
								   mgmt),
		    TP_STRUCT__entry(__field(u16, freq) __field(u8, vif_idx)
				     __field(u8, sta_idx) __field(u16,
								  frame_control)
				     __field(u8, action_cat) __field(u8, action_type) __field(u8, action_p2p)
		    ), TP_fast_assign(__entry->freq = freq;
				      __entry->vif_idx = vif_idx;
				      __entry->sta_idx = sta_idx;
				      __entry->frame_control =
				      mgmt->frame_control;
				      __entry->action_cat =
				      ...
				      __entry->action_p2p =
				      *((u8 *) & mgmt->u.action.category + MGMT_ACTION_OUI_SUBTYPE_OFFSET);
		    ),
		    TP_printk("f=%d vif=%d sta=%d -> %s", __entry->freq,
			      __entry->vif_idx, __entry->sta_idx,
			      __print_mgmt_info(__entry->frame_control,
						__entry->action_cat, __entry->action_type, __entry->action_p2p))
    );

DEFINE_EVENT(mgmt_template, mgmt_tx,
	     TP_PROTO(u16 freq, u8 vif_idx, u8 sta_idx,
		      struct ieee80211_mgmt *mgmt), TP_ARGS(freq, vif_idx, sta_idx, mgmt));

DEFINE_EVENT(mgmt_template, mgmt_rx,
	     TP_PROTO(u16 freq, u8 vif_idx, u8 sta_idx,
		      struct ieee80211_mgmt *mgmt), TP_ARGS(freq, vif_idx, sta_idx, mgmt));

TRACE_EVENT(mgmt_cfm,
	    TP_PROTO(u8 vif_idx, u8 sta_idx, bool acked),
	    TP_ARGS(vif_idx, sta_idx, acked), TP_STRUCT__entry(__field(u8, vif_idx)
							       __field(u8, sta_idx)
							       __field(bool, acked)
	    ), TP_fast_assign(__entry->vif_idx = vif_idx;
			      __entry->sta_idx = sta_idx; __entry->acked = acked;
	    ), TP_printk("vif=%d sta=%d ack=%d", __entry->vif_idx, __entry->sta_idx, __entry->acked)
    );
...
const char *ftrace_print_txq(struct trace_seq *p, int txq_idx)
{
	...
	if (txq_idx == TXQ_INACTIVE) {
		...
		trace_seq_printf(p, "[STA %d/%d]", txq_idx / NX_NB_TXQ_PER_STA, txq_idx % NX_NB_TXQ_PER_STA);
...
const char *ftrace_print_sta(struct trace_seq *p, int sta_idx)
{
	...
	if (sta_idx < asr_hw->sta_max_num) {
		trace_seq_printf(p, "[STA %d]", sta_idx);
	} else {
		trace_seq_printf(p, "[BC/MC %d]", sta_idx - asr_hw->sta_max_num);
	}
...
const char *ftrace_print_hwq(struct trace_seq *p, int hwq_idx)
{
	...
	static const struct trace_print_flags symbols[] = { {ASR_HWQ_BK, "BK"},
	{ASR_HWQ_BE, "BE"},
	{ASR_HWQ_VI, "VI"},
	{ASR_HWQ_VO, "VO"},
	{ASR_HWQ_BCMC, "BCMC"},
	...
	return ftrace_print_symbols_seq(p, hwq_idx, symbols);
}
...
TRACE_EVENT(txq_select,
	    ...
	    TP_ARGS(txq_idx, pkt_ready_up, skb), TP_STRUCT__entry(__field(u16, txq_idx)
								  __field(u16, pkt_ready)
								  __field(struct sk_buff *, skb)
	    ), TP_fast_assign(__entry->txq_idx = txq_idx; __entry->pkt_ready = pkt_ready_up; __entry->skb = skb;
	    ), TP_printk("%s pkt_ready_up=%d skb=%p", __print_txq(__entry->txq_idx), __entry->pkt_ready, __entry->skb)
    );

DECLARE_EVENT_CLASS(hwq_template, TP_PROTO(u8 hwq_idx), TP_ARGS(hwq_idx), TP_STRUCT__entry(__field(u8, hwq_idx)
		    ), TP_fast_assign(__entry->hwq_idx = hwq_idx;
		    ), TP_printk("%s", __print_hwq(__entry->hwq_idx))
    );

DEFINE_EVENT(hwq_template, hwq_flowctrl_stop, TP_PROTO(u8 hwq_idx), TP_ARGS(hwq_idx));

DEFINE_EVENT(hwq_template, hwq_flowctrl_start, TP_PROTO(u8 hwq_idx), TP_ARGS(hwq_idx));

DECLARE_EVENT_CLASS(txq_template, TP_PROTO(struct asr_txq *txq), TP_ARGS(txq), TP_STRUCT__entry(__field(u16, txq_idx)
		    ), TP_fast_assign(__entry->txq_idx = txq->idx;
		    ), TP_printk("%s", __print_txq(__entry->txq_idx))
    );

DEFINE_EVENT(txq_template, txq_add_to_hw, TP_PROTO(struct asr_txq *txq), TP_ARGS(txq));

DEFINE_EVENT(txq_template, txq_del_from_hw, TP_PROTO(struct asr_txq *txq), TP_ARGS(txq));

DEFINE_EVENT(txq_template, txq_flowctrl_stop, TP_PROTO(struct asr_txq *txq), TP_ARGS(txq));

DEFINE_EVENT(txq_template, txq_flowctrl_restart, TP_PROTO(struct asr_txq *txq), TP_ARGS(txq));

TRACE_EVENT(process_txq, TP_PROTO(struct asr_txq *txq), TP_ARGS(txq), TP_STRUCT__entry(__field(u16, txq_idx)
										       __field(u16, len)
	    ...
	    TP_fast_assign(__entry->txq_idx = txq->idx;
			   __entry->len = skb_queue_len(&txq->sk_list); __entry->len_retry = txq->nb_retry;
			   __entry->credit = txq->credits; __entry->limit = txq->push_limit;
	    ),
	    TP_printk("%s txq_credits=%d, len=%d, retry_len=%d, push_limit=%d",
		      __print_txq(__entry->txq_idx), __entry->credit, __entry->len, __entry->len_retry, __entry->limit)
    );

DECLARE_EVENT_CLASS(txq_reason_template,
		    TP_PROTO(struct asr_txq *txq, u16 reason),
		    TP_ARGS(txq, reason), TP_STRUCT__entry(__field(u16, txq_idx)
							   __field(u16, reason)
							   __field(u16, status)
		    ), TP_fast_assign(__entry->txq_idx = txq->idx;
				      __entry->reason = reason; __entry->status = txq->status;
		    ),
		    TP_printk("%s reason=%s status=%s",
			      __print_txq(__entry->txq_idx), __print_symbolic(__entry->reason, {
									      ASR_TXQ_STOP_FULL, "FULL"}, {
									      ASR_TXQ_STOP_CSA, "CSA"}, {
									      ASR_TXQ_STOP_STA_PS, "PS"}, {
									      ASR_TXQ_STOP_VIF_PS, "VPS"}, {
									      ASR_TXQ_STOP_CHAN, "CHAN"}, {

#ifdef CONFIG_TWT
									      ASR_TXQ_STOP_TWT, "TWT"
#else
									      ASR_TXQ_STOP_MU_POS, "MU"
#endif
												...
												ASR_TXQ_NDEV_FLOW_CTRL,
												"FLW_CTRL"}))

    );

DEFINE_EVENT(txq_reason_template, txq_start, TP_PROTO(struct asr_txq *txq, u16 reason), TP_ARGS(txq, reason));

DEFINE_EVENT(txq_reason_template, txq_stop, TP_PROTO(struct asr_txq *txq, u16 reason), TP_ARGS(txq, reason));

TRACE_EVENT(push_desc,
	    TP_PROTO(struct sk_buff *skb, struct asr_sw_txhdr *sw_txhdr,
		     int push_flags), TP_ARGS(skb, sw_txhdr, push_flags),
	    ...
	    TP_fast_assign(__entry->skb = skb;
			   ...
			   __entry->hwq_cred =
			   sw_txhdr->txq->hwq->credits[ASR_TXQ_POS_ID(sw_txhdr->txq)];
			   __entry->flag = sw_txhdr->desc.host.flags;
			   __entry->len = sw_txhdr->desc.host.packet_len; __entry->mu_info = 0;
	    ),
	    TP_printk
	    ("%s skb=%p (len=%d) hw_queue=%s cred_txq=%d cred_hwq=%d %s flag=%s %s%s",
	     __print_txq(__entry->tx_queue), __entry->skb, __entry->len,
	     __print_hwq(__entry->hw_queue), __entry->txq_cred,
	     __entry->hwq_cred, __print_mu_info(__entry->mu_info), __print_flags(__entry->flag, "|", {
										 ...
										 TXU_CNTRL_TDLS, "TDLS"}),
	     (__entry->push_flag & ASR_PUSH_IMMEDIATE) ? "(IMMEDIATE)" : "",
	     (!(__entry->flag & TXU_CNTRL_RETRY) && (__entry->push_flag & ASR_PUSH_RETRY)) ? "(SW_RETRY)" : "")
    );

TRACE_EVENT(txq_queue_skb,
	    TP_PROTO(struct sk_buff *skb, struct asr_txq *txq, bool retry),
	    TP_ARGS(skb, txq, retry), TP_STRUCT__entry(__field(struct sk_buff *, skb)
						       __field(u16, txq_idx)
						       __field(s8, credit)
						       __field(u16, q_len)
	    ...
	    TP_fast_assign(__entry->skb = skb;
			   ...
			   __entry->credit = txq->credits; __entry->q_len = skb_queue_len(&txq->sk_list);
			   __entry->q_len_retry = txq->nb_retry; __entry->retry = retry;
	    ),
	    TP_printk
	    ("%s skb=%p retry=%d txq_credits=%d queue_len=%d (retry = %d)",
	     __print_txq(__entry->txq_idx), __entry->skb, __entry->retry,
	     __entry->credit, __entry->q_len, __entry->q_len_retry)
    );

DECLARE_EVENT_CLASS(idx_template, TP_PROTO(u16 idx), TP_ARGS(idx), TP_STRUCT__entry(__field(u16, idx)
...
TRACE_EVENT(process_hw_queue, TP_PROTO(struct asr_hwq *hwq), TP_ARGS(hwq), TP_STRUCT__entry(__field(u16, hwq)
											    __array(u8, credits,
												    CONFIG_USER_MAX)
	    ...
	    TP_printk("hw_queue=%s hw_credits=%s", __print_hwq(__entry->hwq), __print_hwq_cred(__entry->credits))
    ) ;

DECLARE_EVENT_CLASS(sta_idx_template, TP_PROTO(u16 idx), TP_ARGS(idx), TP_STRUCT__entry(__field(u16, idx)
		    ), TP_fast_assign(__entry->idx = idx;
...
TRACE_EVENT(skb_confirm,
	    TP_PROTO(struct sk_buff *skb, struct asr_txq *txq,
		     struct asr_hwq *hwq, struct tx_cfm_tag *cfm), TP_ARGS(skb,
									   txq,
									   hwq,
									   cfm),
	    TP_STRUCT__entry(__field(struct sk_buff *, skb)
	    ...
	    ), TP_fast_assign(int i; __entry->skb = skb; __entry->txq_idx = txq->idx; __entry->hw_queue = hwq->id;
			      for (i = 0; i < CONFIG_USER_MAX; i++)
	    ...
	    TP_printk
	    ("%s skb=%p hw_queue=%s, hw_credits=%s, txq_credits=%d (+%d)"
	     " ampdu=%d", __print_txq(__entry->txq_idx), __entry->skb,
	     __print_hwq(__entry->hw_queue),
	     __print_hwq_cred(__entry->hw_credit), __entry->sw_credit, __entry->sw_credit_up, __entry->ampdu_size)
    ) ;

TRACE_EVENT(credit_update,
	    TP_PROTO(struct asr_txq *txq, s8 cred_up),
	    TP_ARGS(txq, cred_up), TP_STRUCT__entry(__field(struct sk_buff *, skb)
						    __field(u16, txq_idx)
						    __field(s8, sw_credit)
						    __field(s8, sw_credit_up)
	    ), TP_fast_assign(__entry->txq_idx = txq->idx;
			      __entry->sw_credit = txq->credits; __entry->sw_credit_up = cred_up;
	    ),
	    TP_printk("%s txq_credits=%d (%+d)", __print_txq(__entry->txq_idx),
		      __entry->sw_credit, __entry->sw_credit_up)
    )

DECLARE_EVENT_CLASS(ps_template, TP_PROTO(struct asr_sta *sta), TP_ARGS(sta), TP_STRUCT__entry(__field(u16, idx)
		    ...
		    TP_fast_assign(__entry->idx = sta->sta_idx;
				   __entry->ready_ps =
				   sta->ps.pkt_ready[LEGACY_PS_ID]; __entry->sp_ps = sta->ps.sp_cnt[LEGACY_PS_ID];
		    ...
		    TP_printk("%s [PS] ready=%d sp=%d [UAPSD] ready=%d sp=%d",
			      __print_sta(__entry->idx), __entry->ready_ps,
			      __entry->sp_ps, __entry->ready_uapsd, __entry->sp_uapsd)
    );

DEFINE_EVENT(ps_template, ps_queue, TP_PROTO(struct asr_sta *sta), TP_ARGS(sta));

DEFINE_EVENT(ps_template, ps_push, TP_PROTO(struct asr_sta *sta), TP_ARGS(sta));

DEFINE_EVENT(ps_template, ps_enable, TP_PROTO(struct asr_sta *sta), TP_ARGS(sta));

TRACE_EVENT(ps_traffic_update,
	    TP_PROTO(u16 sta_idx, u8 traffic, bool uapsd),
	    TP_ARGS(sta_idx, traffic, uapsd), TP_STRUCT__entry(__field(u16, sta_idx)
							       __field(u8, traffic)
							       __field(bool, uapsd)
	    ...
	    TP_printk("%s %s%s traffic available ",
		      ...
		      __entry->traffic ? "" : "no more ", __entry->uapsd ? "U-APSD" : "legacy PS")
    );

TRACE_EVENT(ps_traffic_req,
	    TP_PROTO(struct asr_sta *sta, u16 pkt_req, u8 ps_id),
	    TP_ARGS(sta, pkt_req, ps_id), TP_STRUCT__entry(__field(u16, idx)
							   __field(u16, pkt_req)
							   __field(u8, ps_id)
							   __field(u16, ready)
							   __field(u16, sp)
	    ), TP_fast_assign(__entry->idx = sta->sta_idx; __entry->pkt_req = pkt_req; __entry->ps_id = ps_id;
			      __entry->ready = sta->ps.pkt_ready[ps_id]; __entry->sp = sta->ps.sp_cnt[ps_id];
	    ),
...
DECLARE_EVENT_CLASS(ipc_msg_template, TP_PROTO(u16 id), TP_ARGS(id), TP_STRUCT__entry(__field(u16, id)
		    ), TP_fast_assign(__entry->id = id;
		    ), TP_printk("%s (%d - %d)", ASR_ID2STR(__entry->id), MSG_T(__entry->id), MSG_I(__entry->id))
    );
...
#endif /* !defined(_ASR_EVENTS_H) || defined(TRACE_HEADER_MULTI_READ) */

#undef TRACE_INCLUDE_PATH
#undef TRACE_INCLUDE_FILE
#define TRACE_INCLUDE_PATH ASR_EVENT_PATH
#define TRACE_INCLUDE_FILE asr_events
