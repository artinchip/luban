...
int rtw_get_bcn_keys(_adapter *adapter, u8 *whdr, u32 flen, struct beacon_keys *bcn_keys)
{
	return _rtw_get_bcn_keys(
		...
		, BSS_EX_MBSSID_IDX(&adapter->mlmeextpriv.mlmext_info.network)
		, bcn_keys);
}
...
int rtw_check_bcn_info(ADAPTER *Adapter, u8 *pframe, u32 packet_len)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_CSA_UPDATE_BEACON)) {
		...
		*/
		...
		rtw_ecsa_update_sta_mlme(Adapter, pframe, packet_len);
...
#endif
	{
		...
		if (recv_beacon.ssid_len != cur_beacon->ssid_len) {
			...
			RTW_WARN("%s: SSID length changes, new %d old %d\n", __func__,
					recv_beacon.ssid_len, cur_beacon->ssid_len);
			RTW_INFO_DUMP("[old ssid]: ", cur_beacon->ssid, cur_beacon->ssid_len);
			RTW_INFO_DUMP("[new ssid]: ", recv_beacon.ssid, cur_beacon->ssid_len);
		} else if ((_rtw_memcmp(recv_beacon.ssid,
					cur_beacon->ssid,
					cur_beacon->ssid_len) == _FALSE)) {
			pmlmeinfo->illegal_beacon_code |= SSID_CHANGED;
			RTW_WARN("%s: SSID changed\n", __func__);
			RTW_INFO_DUMP("[old ssid]: ", cur_beacon->ssid, cur_beacon->ssid_len);
			RTW_INFO_DUMP("[new ssid]: ", recv_beacon.ssid, cur_beacon->ssid_len);
		}
...
static int _rtw_ies_get_supported_rate(u8 *sup_r_ie, sint sup_r_ie_len
...
int rtw_ies_get_supported_rate(u8 *ies, uint ies_len, u8 *rate_set, u8 *rate_num)
{
	...
	return _rtw_ies_get_supported_rate(sup_r_ie, sup_r_ie_len
