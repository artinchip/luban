		    ...
		    _adapter *padapter, bool update_ie)
{
...
#endif
	long rssi_new = src->Rssi;
	long rssi_ori = dst->Rssi;
	u8 sq_smp = src->PhyInfo.SignalQuality;
	u8 ss_final = 0;
	u8 sq_final = 0;
	long rssi_final = 0;
	u8 dst_rssi_is_valid = true;
	u8 update_case = 0;
	...
	if (src->PhyInfo.isValid && dst->PhyInfo.isValid && sq_smp != 101) {
		/* Receiving the valid RSSI from correct channel and the RSSI stored in scan queue is also valid */
		update_case = 1;
	} else if (src->PhyInfo.isValid && !dst->PhyInfo.isValid) {
		/* Replace the invalid RSSI stored in scan queue with the received valid RSSI even not from correct channel */
		update_case = 2;
	} else {
		/* Don't update RSSI, use original RSSI */
		update_case = 3;
	}
	...
	if (check_fwstate(&padapter->mlmepriv, WIFI_ASOC_STATE) && is_same_network(&(padapter->mlmepriv.cur_network.network), src, 0)) {
		...
		switch (update_case) {
		case 1:
			rssi_final = (rssi_new + rssi_ori * 4) / 5;
			break;
		case 2:
			rssi_final = rssi_new;
			break;
		case 3:
			rssi_final = rssi_ori;
			dst_rssi_is_valid = dst->PhyInfo.isValid; /* backup is_valid of dst */
			break;
		}
	} else {
		switch (update_case) {
	...
	dst->Rssi = rssi_final;
	dst->PhyInfo.isValid = dst_rssi_is_valid;
...
void rtw_free_assoc_resources(_adapter *adapter, u8 lock_scanned_queue)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)) {
		...
		if (ptdlsinfo->link_established == _TRUE)
			rtw_tdls_cmd(adapter, NULL, TDLS_RS_RCR, 0);
#endif /* CONFIG_TDLS */
...
#if defined(CONFIG_CONCURRENT_MODE) && defined(CONFIG_AP_MODE)
{
	...
	if (rfctl->ap_csa_cnt_input > 0 && ifbmp && MSTATE_AP_LD_NUM(&mstate) &&
...
#if defined(CONFIG_CONCURRENT_MODE) && defined(CONFIG_AP_MODE)
	/* Trigger CSA */
	if (trigger_csa && candidate != NULL) {
		u8 csa_band = 0, csa_ch = 0, csa_bw = 0, csa_offset = 0;
		u8 csa_ch_width = 0xff, seg_0 = 0, seg_1 = 0;
		u8 csa_switch_cnt = 0, csa_mode = 0;
		u8 ecsa_op_class = 0;

		csa_band = (csa_ch > 14) ? BAND_ON_5G : BAND_ON_24G;
		rtw_bss_get_chbw(&(candidate->network),
			&csa_ch, &csa_bw, &csa_offset, 1, 1);
		csa_switch_cnt = rfctl->ap_csa_cnt_input;

		/* Store candidata network until softap switch channel done */
		_rtw_memcpy(&(rfctl->csa_candidate_network), candidate, sizeof(struct wlan_network));

		RTW_INFO("CSA : In the concurrent mode of STA+AP, "
			"the CSA process is triggered before the STA joins the BSS.\n");

		rtw_hal_trigger_csa_start(adapter, CSA_AP_CORE_SWITCH_CH,
...
#ifdef CONFIG_80211N_HT
void	rtw_ht_use_default_setting(_adapter *padapter, bool log)
{
	struct mlme_priv		*pmlmepriv = &padapter->mlmepriv;
	struct ht_priv		*phtpriv = &pmlmepriv->htpriv;
	struct registry_priv	*pregistrypriv = &padapter->registrypriv;
...
unsigned int rtw_restructure_ht_ie(_adapter *padapter, u8 *in_ie, u8 *out_ie, uint in_len, uint *pout_len, u8 channel, bool log)
{
	u32 ielen, out_len;
