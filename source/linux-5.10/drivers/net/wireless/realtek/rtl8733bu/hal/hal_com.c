...
void dump_chip_info(HAL_VERSION	ChipVersion)
{
	...
	else if (IS_8733B_SERIES(ChipVersion))
		cnt += sprintf((buf + cnt), "Chip Version Info: CHIP_8733B_");
...
int c2h_mac_hidden_rpt_2_hdl(_adapter *adapter, u8 *data, u8 len)
{
	...
	#if defined(CONFIG_8733B)
	if (IS_8733B_SERIES(hal_data->version_id)) {
...
bool rtw_read_from_frame_mask(_adapter *adapter, u8 idx)
{
	...
	if(_rtw_wow_chk_cap(adapter, WOW_CAP_HALMAC_ACCESS_PATTERN_IN_TXFIFO)) {
		/* 8733B cannot indirect access tx fifo
...
u64 rtw_hal_get_tsftr_by_port(_adapter *adapter, u8 port)
{
	...
	|| defined(CONFIG_RTL8822C) || defined(CONFIG_RTL8733B) || defined(CONFIG_RTL8822E)
	case RTL8814A:
	case RTL8822B:
	case RTL8821C:
	case RTL8822C:
...
int hal_efuse_macaddr_offset(_adapter *adapter)
{
	...
	case RTL8733B:
		if (interface_type == RTW_USB)
			addr_offset = EEPROM_MAC_ADDR_8733BU;
		else if (interface_type == RTW_SDIO)
			addr_offset = EEPROM_MAC_ADDR_8733BS;
		break;
...
void dm_DynamicUsbTxAgg(_adapter *padapter, u8 from_timer)
{
	...
	if (IS_HARDWARE_TYPE_8822BU(padapter) || IS_HARDWARE_TYPE_8821CU(padapter) 
		...
		|| IS_HARDWARE_TYPE_8733BU(padapter) || IS_HARDWARE_TYPE_8822EU(padapter))
...
#ifdef CONFIG_RTL8733B
	case RTL8733B:
		rtl8733b_init_hal_spec(adapter);
		break;
#endif
#ifdef CONFIG_RTL8822E
	case RTL8822E:
