		...
		char *start, char *stop, struct iw_event *iwe)
{
	...
	if (pbuf) {
		...
		wpa_ie = (u8 *)rtw_zmalloc(255);
		if (!wpa_ie)
			goto exit;
		rsn_ie = (u8 *)rtw_zmalloc(255);
		if (!rsn_ie)
			goto exit;
...
exit:
		if (wpa_ie)
			rtw_mfree(wpa_ie, 255);
		if (rsn_ie)
			rtw_mfree(rsn_ie, 255);
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	if (check_fwstate(pmlmepriv, wait_status) == _TRUE)
		return -EAGAIN;
#else
#ifdef CONFIG_P2P
	if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE))
		...
		union iwreq_data *wrqu, char *extra, char *subcmd)
{
	...
	RTW_INFO("[%s] data = %s\n", __FUNCTION__, subcmd);
	attr_content = (u8 *)rtw_zmalloc(100);
	if (!attr_content)
		return -ENOMEM;
	macstr2num(peerMAC, subcmd);
		      ...
		      union iwreq_data *wrqu, char *extra)
{
	...
	#endif
	} else if (_rtw_memcmp(extra, "wow_ips=", 8)) {
		if (sscanf(extra + 8, "%u", &mode) > 0)
			ret = rtw_pm_set_wow_ips(padapter, mode);
	}
#endif /* CONFIG_WOWLAN */
	else
		ret = -EINVAL;
			    ...
			    union iwreq_data *wdata, char *extra)
{
	...
	err = 0;
	mask_buf = (u8 *)rtw_zmalloc(64);
	if (!mask_buf) {
		err = -ENOMEM;
		goto exit;
	}
	data = rtw_zmalloc(EFUSE_MAX_MAP_LEN);
	if (data == NULL) {
		err = -ENOMEM;
		goto exit;
	}
	rawdata = rtw_zmalloc(EFUSE_MAX_MAP_LEN);
	if (rawdata == NULL) {
		err = -ENOMEM;
		goto exit;
	}
...
exit:
	if (data)
		rtw_mfree(data, EFUSE_MAX_MAP_LEN);
	if (rawdata)
		rtw_mfree(rawdata, EFUSE_MAX_MAP_LEN);
	if (mask_buf)
		rtw_mfree(mask_buf, 64);
	if (!err)
			    ...
			    union iwreq_data *wdata, char *extra)
{
	...
	*(extra + wrqu->length) = '\0';
	...
	ShadowMapBT = rtw_malloc(EFUSE_MAX_MAP_LEN);
	if (ShadowMapBT == NULL) {
		err = -ENOMEM;
		goto exit;
	}
	...
	setrawdata = rtw_malloc(EFUSE_MAX_HW_SIZE);
	if (setrawdata == NULL) {
		err = -ENOMEM;
		goto exit;
	}
...
exit:
	...
	if (ShadowMapBT)
		rtw_mfree(ShadowMapBT, EFUSE_MAX_MAP_LEN);
	if (ShadowMapWiFi)
		rtw_mfree(ShadowMapWiFi, wifimaplen);
	if (setrawdata)
		rtw_mfree(setrawdata, EFUSE_MAX_HW_SIZE);
			      ...
			      union iwreq_data *wrqu, char *extra)
{
	...
	if (take_care_iqk == _TRUE) {
		...
		rtw_tdls_cmd(padapter, ptdls_sta->cmn.mac_addr, TDLS_CH_SW_PREPARE, 0);
#else
		u8 central_chnl;
		u8 bw_mode;
...
#include <../../hal/rtl8733b/rtl8733b.h>

#define REG_LOOPBACK_ENABLE 0x0103
#define LOOKBACK_ENABLE_VALUE 0x0b
#define cal_txdesc_chksum(padapter, desc) rtl8733b_cal_txdesc_chksum(padapter, desc)
#define dump_txdesc_data(padapter, desc) rtl8733b_dbg_dump_tx_desc(padapter, DATA_FRAMETAG, desc);
#define get_rx_desc(rx_desc, rxbuf) rtl8733b_rxdesc2attribute(rx_desc, rxbuf)
#define hal_init rtl8733b_hal_init
