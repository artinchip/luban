...
struct rf_ctl_t {
...
#ifdef CONFIG_ECSA
#ifdef PRIVATE_R
	u8 p2p_park_ch; /* decide the parking channel of GO after leaving DFS channel */
#endif
#ifdef CONFIG_AP_MODE
	u8 ap_csa_mode;
	u8 ap_ecsa_op_class;
	...
	u8 ap_csa_band;
	u8 ap_csa_bw;
	u8 ap_csa_ch_offset; /* please refer to sec_ch_offset_ie */
	u8 ap_csa_ch_width;
	u8 ap_csa_ch_freq_seg0;
	u8 ap_csa_ch_freq_seg1;
	u8 ap_csa_en;
	u8 ap_csa_wait_update_bcn; /* wait beacon update */
#ifdef CONFIG_CONCURRENT_MODE
	u8 ap_csa_cnt_input; /* Input from proc, default value is DEFAULT_CSA_CNT */
	struct wlan_network csa_candidate_network;
#endif
	...
	u8 csa_switch_cnt;
	u8 csa_band;
	u8 csa_bw; /* core layer bandwidth not channel width */
	u8 csa_ch_offset; /* please refer to sec_ch_offset_ie */
	u8 csa_ch_width;
...
#ifdef CONFIG_ECSA
#define IS_ECSA_RUNNING(adapter) rtw_mr_is_ecsa_running(adapter_to_rfctl(adapter))
#define IS_ECSA_STOP_TX(rfctl) ((rfctl)->csa_mode == CSA_STOP_TX)
#else
#define IS_ECSA_RUNNING(adapter) 0
#define IS_ECSA_STOP_TX(rfctl) 0
#endif /* CONFIG_ECSA */
