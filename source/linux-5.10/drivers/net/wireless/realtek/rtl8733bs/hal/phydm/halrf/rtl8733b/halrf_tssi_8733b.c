...
u32 _halrf_tssi_get_targetpower_8733b(void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	u32 mask;
	s32 pout;
	...
	value0 = (s8)(odm_get_bb_reg(dm, rate_reg, mask) & 0xff);
	pout = (s32)value0 * 100 / 4 +1600;
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[TSSI]tx_rate = 0x%x,targetpwr = %d (/100)\n",
		tx_rate, pout);
...
u32 halrf_get_online_tssi_de_8733b(void *dm_void, u8 path, s32 pout)
{
	...
	db_temp = _halrf_tssi_get_targetpower_8733b(dm);

	RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[TSSI]pout = 0x%x\n", pout);

	if(pout > 22000){
		pout=22000;
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[TSSI] Out of range,pout2 = 0x%x\n", pout);
	}

	de = ((pout - db_temp * 10) * 8) / 1000;
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[RF][TSSI] pout = 0x%x,de = 0x%x\n", pout, de);
...
void _halrf_tssi_anapar_8733b(void *dm_void, u8 path)
{
	...
	odm_set_bb_reg(dm, R_0x1e1c, 0x0000F000, 0xB);//0xc ADC 160M,0x8 ADC 10M,
}
...
void halrf_tssi_dck_8733b(void *dm_void)
{
	...
	odm_set_bb_reg(dm, R_0x4328, BIT(30), 0x0);
	odm_set_bb_reg(dm, R_0x432c, 0x000000FF, 0x55);/*0x51*/
	//odm_set_bb_reg(dm, R_0x4368, MASKDWORD, 0x000003fe);
	odm_set_bb_reg(dm, R_0x4368, MASKDWORD, 0x00000002);
	odm_set_bb_reg(dm, R_0x4378, MASKDWORD, 0x00000002);
...
void _halrf_tssi_set_txpwr_bb_com_8733b(void *dm_void, u8 path)
{
	...
	odm_set_bb_reg(dm, R_0x4308, MASKDWORD, 0x5c545c50);
	...
	odm_set_bb_reg(dm, R_0x1c84, 0x000003c0, 0x1);/*cck*/
}
...
void _halrf_set_power_base_8733b(void *dm_void, u8 path)
{
	...
	u8 channel = *dm->channel, bandwidth = *dm->band_width;
	u8  txagc, tssi_offset, txagc_bb, rf_reg18, txagc_base;
	u8 i = 0, backup_num = 5;
	s8 tmp[2] = {0};
	...
	txagc_bb = (u8)odm_get_bb_reg(dm, R_0x42f0, MASKBYTE1);
	txagc_base = (u8)odm_get_bb_reg(dm, R_0x4308, MASKBYTE1)/4;
	...
	tssi->txagc_offset_thermaltrack[path] =
		 8 * (txagc + tssi_offset -txagc_base) + txagc_bb;
	...
	if ((!(dm->rfe_type <= 2 ||
		...
		txagc_base = (u8)odm_get_bb_reg(dm, R_0x4308, MASKBYTE3)/4;
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
...
void halrf_do_tssi_8733b(void *dm_void)
{
	...
	struct _halrf_tssi_data *tssi = &rf->halrf_tssi_data;
	u32 backup_num = 6;
	u32 bb_reg[6] = {R_0x1c38, R_0x1860, R_0x1d40,
				  R_0x1e70, R_0x522, R_0x4384};
	...
	offset = (s8)odm_get_bb_reg(dm, R_0x3a10, MASKBYTE3);//MCS7
