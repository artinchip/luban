...
bool ssv6200_is_scan_allow(struct ssv_softc *sc)
{
    if (!sc->sh->cfg.ap_mode_scan_enable && !IS_NON_AP_MODE && 
        !IS_NONE_STA_CONNECTED_IN_AP_MODE) {
        return false;
    }
    ...
    if (changed & IEEE80211_CONF_CHANGE_CHANNEL) {
        ...
        if (hw->conf.flags & IEEE80211_CONF_OFFCHANNEL)
        {
            ...
            if (/* (IS_ALLOW_SCAN || IS_NON_AP_MODE || IS_NONE_STA_CONNECTED_IN_AP_MODE)
                    ...
                    && */ssv6200_is_scan_allow(sc) && ((sc->hw_chan != chan->hw_value) || (sc->hw_chan_type != channel_type)) )
            { 
...
#define MAX_RFPHY_REG_VERSION_LEN   8
    ...
    switch (h_evt->h_event) {
        ...
        case SOC_EVT_TEMPERATURE:
            ...
            dev_kfree_skb_any(skb);
            break;
        case SOC_EVT_SSV_NIMBLE:
            {
                int ssv_ctl_from_ssv_nimble(char *pData, int len);
                ssv_ctl_from_ssv_nimble((char *)h_evt->dat,h_evt->len-sizeof(struct cfg_host_event));
                dev_kfree_skb_any(skb);
            }
            break;
        default:
...
void ssv6xxx_ble_api_send(struct ssv_softc *sc, char *buf, u16 buflen)
{
    struct sk_buff *skb = NULL;
    struct cfg_host_cmd *host_cmd = NULL;
    int ret = 0;

    skb = ssv_skb_alloc(sc, HOST_CMD_HDR_LEN + buflen);
    if (skb == NULL) {
        printk("%s(): Fail to alloc cmd buffer.\n", __FUNCTION__);
        return;
    }

    skb_put(skb, HOST_CMD_HDR_LEN + buflen);
    host_cmd = (struct cfg_host_cmd *)skb->data;
    memset(host_cmd, 0x0, sizeof(struct cfg_host_cmd));
    host_cmd->c_type = HOST_CMD;
    host_cmd->h_cmd = (u8)SSV6XXX_HOST_CMD_SSV_NIMBLE;
    host_cmd->len = HOST_CMD_HDR_LEN + buflen;
    memcpy(host_cmd->un.dat8, buf, buflen);
    
    //ssv6xxx_tx_flow_control(sc, true);
    ret = HCI_SEND_CMD(sc->sh, skb);
    if (ret < 0)
        printk("Fail to send private command\n");
}
