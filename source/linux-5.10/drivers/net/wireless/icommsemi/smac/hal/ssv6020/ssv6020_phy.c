...
static bool ssv6xxx_is_speaci_frame(struct ssv_softc *sc, struct ieee80211_hdr *hdr)
{
#define SSV_SNAP_SIZE   6
#define SSV_PROTOC_TYPE_SIZE    2
    ...
    switch (ether_type) {
        ...
        case ETH_P_ARP:
            ...
            break;
        case ETH_P_IP:
            ip =  (struct iphdr *)((u8 *)hdr + offset + SSV_PROTOC_TYPE_SIZE);
	    if (IPPROTO_UDP == ip->protocol) {
                udp = (struct udphdr *)((u8 *)ip + (ip->ihl << 2));
                if (((ntohs(udp->dest) == 67) && (ntohs(udp->source) == 68)) ||
                        ((ntohs(udp->source) == 67) && (ntohs(udp->dest) == 68))) {
			printk("sssssdebug for speacial rate frame\n");
                        retval = true;
                }
	    }
            break;
...
static void ssv6020_update_txinfo (struct ssv_softc *sc, struct sk_buff *skb)
{
    ...
    if(ssv6xxx_is_speaci_frame(sc,hdr)){
		printk(" set speaci rate\n");
        tx_desc->sw_speaci_rate = 1;
    }
