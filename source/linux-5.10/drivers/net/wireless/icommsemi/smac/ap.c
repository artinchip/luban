...
#ifdef CONFIG_MIFI
    ptr->mifi = 1;
    ptr->max_idle_time = sh->cfg.mifi_idle_timeout; 
#endif
...
void ssv6xxx_beacon_set(struct ssv_softc *sc, struct sk_buff *beacon_skb, int dtim_offset, 
                bool ssid_hidden, int ssid_len, u8 *ssid)
#else
void ssv6xxx_beacon_set(struct ssv_softc *sc, struct sk_buff *beacon_skb, int dtim_offset)
#endif				
{
...
#ifdef CONFIG_MIFI
    ptr->mifi = 1;
    ptr->ssid_hidden = (ssid_hidden == true) ? 1 : 0;
    ptr->ssid_len = ssid_len;
    memcpy(ptr->ssid, ssid, ssid_len);
#endif
...
//	printk("[A] ssv6xxx_beacon_change\n");
    ...
    //	
    	...
    	ssv6xxx_beacon_set(sc, skb, tim_offset, 
            vif->bss_conf.hidden_ssid, vif->bss_conf.ssid_len, vif->bss_conf.ssid);
	    #else
		ssv6xxx_beacon_set(sc, skb, tim_offset);
		#endif
    }while(0);
