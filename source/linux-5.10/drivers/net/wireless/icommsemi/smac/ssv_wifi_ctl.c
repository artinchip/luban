...
static struct nla_policy ssv_ctl_genl_policy[SSV_CTL_ATTR_MAX + 1] = {
    ...
    [SSV_CTL_ATTR_SSV_NIMBLE_ENABLE]    = { .type = NLA_U32 },
    [SSV_CTL_ATTR_SSV_NIMBLE_DISABLE]   = { .type = NLA_U32 },
    [SSV_CTL_ATTR_TO_SSV_NIMBLE]        = { .type = NLA_BINARY, .len = IEEE80211_MAX_DATA_LEN },
    [SSV_CTL_ATTR_FROM_SSV_NIMBLE]      = { .type = NLA_BINARY, .len = IEEE80211_MAX_DATA_LEN },
};
#endif
...
int ssv_ctl_ssv_nimble_enable(struct sk_buff *skb, struct genl_info *info)
{
    u32 enable;
    ...
    if (!info->attrs[SSV_CTL_ATTR_SSV_NIMBLE_ENABLE])
    {
    ...
    {
        enable = nla_get_u32(info->attrs[SSV_CTL_ATTR_SSV_NIMBLE_ENABLE]);
#ifdef SSV_CTL_DEBUG
        printk("ssv ble api enable=%d\n", enable);
#endif
    }
...
#endif
    ssv_nimble_enable=1;
    printk(KERN_ERR "\33[31m%s():pid=%d \33[0m\r\n",__FUNCTION__ ,ssv_nimble_usr_pid);
    return 0;

}

int ssv_ctl_ssv_nimble_disable(struct sk_buff *skb, struct genl_info *info)
{
    u32 enable;
    ...
    if (!info->attrs[SSV_CTL_ATTR_SSV_NIMBLE_DISABLE])
    {
        printk("the attrs is not enable\n");
        return -EINVAL;
    }
    ...
    {
        enable = nla_get_u32(info->attrs[SSV_CTL_ATTR_SSV_NIMBLE_DISABLE]);
#ifdef SSV_CTL_DEBUG
        printk("ssv ble api enable=%d\n", enable);
#endif
    }
...
#endif
    ssv_nimble_enable=0;
    printk(KERN_ERR "\33[31m%s()\33[0m\r\n",__FUNCTION__);
    return 0;
...
//    mutex_unlock(&ssv_smartlink_sc->mutex);
    printk(KERN_ERR "\33[31m%s():%d \33[0m\r\n",__FUNCTION__ ,__LINE__);
    printk(KERN_ERR "\33[31m%s():%d \33[0m\r\n",__FUNCTION__ ,__LINE__);
    printk(KERN_ERR "\33[31m%s():%d \33[0m\r\n",__FUNCTION__ ,__LINE__);
    printk(KERN_ERR "\33[31m%s():%d \33[0m\r\n",__FUNCTION__ ,__LINE__);
    return retval;
...
int ssv_ctl_from_ssv_nimble(char *pData, int len)
{
    ...
    msg_head = genlmsg_put(skb, 0, 0, &ssv_ctl_gnl_family, 0, SSV_CTL_CMD_FROM_SSV_NIMBLE);
    if (msg_head == NULL) 
    {
        retval = -ENOMEM;
    ...
    /* fill with message */ 
    retval = nla_put(skb, SSV_CTL_ATTR_FROM_SSV_NIMBLE, inBufLen, pOutBuf);
    if (retval != 0)
    {
        printk("Fail to add attribute in message\n");
        goto free_msg;
...
static int ssv_ctl_to_ssv_nimble(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (!info->attrs[SSV_CTL_ATTR_TO_SSV_NIMBLE])
        return -EINVAL;
    else 
    {
        strlen = nla_len(info->attrs[SSV_CTL_ATTR_TO_SSV_NIMBLE]);
        inval = (char *)nla_data(info->attrs[SSV_CTL_ATTR_TO_SSV_NIMBLE]);
    }


    mutex_lock(&sc->mutex);
    ssv6xxx_ble_api_send(sc, inval, strlen);
...
struct genl_ops ssv_ctl_gnl_ops[] = {
    ...
    {
        .cmd = SSV_CTL_CMD_SSV_NIMBLE_ENABLE,
        .flags = 0,
#if LINUX_VERSION_CODE < KERNEL_VERSION(5,2,0)        
        .policy = ssv_ctl_genl_policy,
#endif        
    ...
    {
        .cmd = SSV_CTL_CMD_SSV_NIMBLE_DISABLE,
        .flags = 0,
#if LINUX_VERSION_CODE < KERNEL_VERSION(5,2,0)        
        .policy = ssv_ctl_genl_policy,
#endif        
