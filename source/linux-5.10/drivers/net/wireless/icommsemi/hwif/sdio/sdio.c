...
#define SSV_TURISMOE_VENDOR_ID      (0x5653)
#define SSV_TURISMOE_DEVID          (0x2060)
...
#endif
{
    {SDIO_DEVICE(SSV_TURISMOE_VENDOR_ID, SSV_TURISMOE_DEVID)},
    {}
};
        ...
        while (remaining) {
            ...
            if (((unsigned long)skb->data) & (PLATFORM_DEF_DMA_ALIGN_SIZE - 1)) {
                ...
                if (remaining > SDIO_DMA_BUFFER_LEN) {
                    printk(KERN_ERR"SDIO Write: unalignmen!!!!!, but aggrate buffer is alignment\n");
                    BUG_ON(1);
                } else {
...
*/
    ...
    ret = __ssv6xxx_sdio_write_reg(glue, ADR_PMU_RAM_02, 0);
...
#endif
{
    ...
    if ( glue )
    {
        ...
        // fw enter dormant  
        ret = __ssv6xxx_sdio_write_reg(glue, ADR_PMU_RAM_02, 0xFFFFFFFF); //lyh
