...
void ssv_gatts_profile_cb(ssv_gatts_cb_event_t event,ssv_gatt_if_t gatts_if,ssv_ble_gatts_cb_param_t *param)
{
    switch(event)
    {
        ...
        case SSV_GATTS_DISCONNECT_EVT:
            {
                printf("SSV_GATTS_DISCONNECT_EVT, conn_id %d, %02x:%02x:%02x:%02x:%02x:%02x, reason %d\n", 
                    param->disconnect.conn_id,
                    param->disconnect.remote_bda[5], param->disconnect.remote_bda[4], param->disconnect.remote_bda[3],
                    param->disconnect.remote_bda[2], param->disconnect.remote_bda[1], param->disconnect.remote_bda[0],
        ...
        case SSV_GATTS_START_EVT:
            printf("SSV_GATTS_START_EVT, status %d,service_handle %d\n", 
                    param->start.status,
