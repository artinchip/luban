...
int ssv_netlink_send_cmd(char *buf, int buflen)
{
    return _netlink_send_cmd(g_gnl_fd, buf, buflen, (int)SSV_CTL_CMD_TO_SSV_NIMBLE, (int)SSV_CTL_ATTR_TO_SSV_NIMBLE);
}
...
int ssv_netlink_start(void)
{
    ...
    _netlink_send_cmd(g_gnl_fd, (char *)&val, (int)sizeof(val), (int)SSV_CTL_CMD_SSV_NIMBLE_ENABLE, (int)SSV_CTL_ATTR_SSV_NIMBLE_ENABLE);
    ret = g_gnl_fd;
out:
    return ret;
}
...
{
    int val = 1;
    _netlink_send_cmd(g_gnl_fd, (char *)&val, (int)sizeof(val), (int)SSV_CTL_CMD_SSV_NIMBLE_DISABLE, (int)SSV_CTL_ATTR_SSV_NIMBLE_DISABLE);
