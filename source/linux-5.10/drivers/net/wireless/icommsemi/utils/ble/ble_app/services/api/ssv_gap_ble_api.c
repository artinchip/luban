...
int ssv_ble_gap_set_scan_swlist(uint8_t action, uint8_t type, uint8_t index, uint8_t *tag, uint8_t len, uint8_t addr_type)
{
    ...
    ST_NIMBLE_SET_SCAN_SWLIST *ptag=NULL;
    unsigned int size = sizeof(ST_NIMBLE_CMD)+sizeof(ST_NIMBLE_SET_SCAN_SWLIST);
    pbleApi=malloc(size);
    if(NULL!=pbleApi)
    {
        memset(pbleApi,0,sizeof(size));
        pbleApi->cmdid=E_SSV_HAL_BLE_GAP_SET_SCAN_SWLIST;
        pbleApi->datalen=sizeof(ST_NIMBLE_SET_SCAN_SWLIST);
        ptag=(ST_NIMBLE_SET_SCAN_SWLIST *)pbleApi->data;
        if(action > SCAN_SWLIST_ACTION_CLR) goto error_exit;
        ptag->action=action;
        ...
        if(len == 0)
        {
            memset(&ptag->target[1], 0, SCAN_SWLIST_NAME_MAX_LENGTH);
        }
        else
        {
