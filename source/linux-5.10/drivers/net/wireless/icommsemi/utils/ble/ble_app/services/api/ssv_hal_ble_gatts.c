...
ssv_err_t ssv_ble_gatts_startstop_service(uint16_t service_handle, uint8_t is_start)
{
    ST_NIMBLE_CMD *bleApi;
    struct gatts_start_service {
        uint16_t            service_handle;
        uint8_t             start_stop;
    } *pstartservice;

    bleApi = (ST_NIMBLE_CMD*)malloc(sizeof(ST_NIMBLE_CMD)+sizeof(struct gatts_start_service));
    if(NULL == bleApi) {
        printf("[%s] malloc(%u) failed!!\n", __func__, (uint32_t)(sizeof(ST_NIMBLE_CMD)+sizeof(struct gatts_start_service)));
        return -1;
    }

    pstartservice = (struct gatts_start_service*)bleApi->data;
    pstartservice->service_handle = service_handle;

    if (is_start)
        pstartservice->start_stop  = 1;
    else
        pstartservice->start_stop  = 0;

    bleApi->datalen = sizeof(struct gatts_start_service);
    bleApi->cmdid = E_SSV_HAL_BLE_GATTS_START_SERVICE;
    ssv_netlink_send_cmd((char *)bleApi, sizeof(ST_NIMBLE_CMD)+sizeof(struct gatts_start_service));
    free(bleApi);
...
int ssv_hal_ble_gatts_create_service_by_db(ssv_gatts_attr_db_t *gatts_attr_db, uint8_t max_nb_attr, uint16_t *out_handle)
{
    ...
    pService = (struct gatts_create_service*)bleApi->data;
    pService->gatts_attr_db = gatts_attr_db;//ptr_dummy; //gatts_attr_db
    pService->max_nb_attr   = max_nb_attr;
    pService->out_handle    = out_handle;//ptr_dummy; //out_handle
    ...
    for(i=0; i<max_nb_attr; i++)
    {
        ((soc_gatts_attr_db_t*)psrc)->attr_control          = gatts_attr_db[i].attr_control;

        ((soc_gatts_attr_db_t*)psrc)->att_desc.uuid_length  = gatts_attr_db[i].att_desc.uuid_length;
        ((soc_gatts_attr_db_t*)psrc)->att_desc.uuid_p       = gatts_attr_db[i].att_desc.uuid_p;
        ((soc_gatts_attr_db_t*)psrc)->att_desc.perm         = gatts_attr_db[i].att_desc.perm;
        ((soc_gatts_attr_db_t*)psrc)->att_desc.max_length   = gatts_attr_db[i].att_desc.max_length;
        ((soc_gatts_attr_db_t*)psrc)->att_desc.length       = gatts_attr_db[i].att_desc.length;
        ((soc_gatts_attr_db_t*)psrc)->att_desc.value        = gatts_attr_db[i].att_desc.value;

        //memcpy(psrc, (char*)gatts_attr_db, max_nb_attr*sizeof(ssv_gatts_attr_db_t));
        psrc += sizeof(soc_gatts_attr_db_t);
    }
...
uint16_t ssv_hal_ble_gatts_send_indication(uint16_t attrib_offset, uint8_t *data, uint16_t len, bool need_confirm)
{
    ...
    pIndication = (struct gatts_send_indication*)bleApi->data;
    pIndication->attrib_offset  = attrib_offset;
    pIndication->data           = data;//ptr_dummy; //data
    pIndication->len            = len;
                                        ...
                                        uint32_t trans_id, ssv_gatt_status_t status, ssv_gatt_rsp_t *rsp)
{
    ...
    pResponse->trans_id = trans_id;
    pResponse->status   = status;
    pResponse->rsp      = rsp;//ptr_dummy; //rsp
...
#endif //__WORDSIZE == 32
ssv_err_t ssv_ble_gatts_start_service(uint16_t service_handle)
{
    return ssv_ble_gatts_startstop_service(service_handle, 1);
}

ssv_err_t ssv_ble_gatts_stop_service(uint16_t service_handle)
{
    return ssv_ble_gatts_startstop_service(service_handle, 0);
}
