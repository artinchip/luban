...
static const uint8_t at_char_prop_read_write_notify   = SSV_GATT_CHAR_PROP_BIT_WRITE | SSV_GATT_CHAR_PROP_BIT_READ | SSV_GATT_CHAR_PROP_BIT_NOTIFY | SSV_GATT_CHAR_PROP_BIT_INDICATE;

static uint16_t ccc_val;
...
static ssv_gatts_attr_db_t gatt_db[AT_HRS_IDX_NB] =
{
    ...
    [AT_IDX_CHAR_VAL_A] =
    {{SSV_GATT_RSP_BY_APP}, {SSV_UUID_LEN_128, (uint8_t *)&AT_GATTS_CHAR_UUID_TEST_A, SSV_GATT_PERM_READ | SSV_GATT_PERM_WRITE,
      AT_GATTS_CHAR_VAL_LEN_MAX, sizeof(at_char_value), (uint8_t *)at_char_value}},
    
    /* Characteristic Client Config Value */
    [AT_IDX_CHAR_CCC_VAL_A] =
    {{SSV_GATT_AUTO_RSP}, {SSV_UUID_LEN_16, (uint8_t *)&at_character_client_config_uuid, SSV_GATT_PERM_READ | SSV_GATT_PERM_WRITE,
    ...
    /* Characteristic Value */
    [AT_IDX_CHAR_VAL_B]  =
    {{SSV_GATT_AUTO_RSP}, {SSV_UUID_LEN_128, (uint8_t *)&AT_GATTS_CHAR_UUID_TEST_B, SSV_GATT_PERM_READ | SSV_GATT_PERM_WRITE,
      AT_GATTS_CHAR_VAL_LEN_MAX, sizeof(at_char_value), (uint8_t *)at_char_value}},
};
...
static void cmd_scanswlist(int argc, char *argv[])
{
    uint8_t action_val;
    uint8_t index_val;
    uint8_t type_val;
    ...
    uint8_t i = 0;
    char *tag_str = NULL;
    uint8_t mac_type = 0xff;
    
    if(argc <= 2 || argc >= 7)
    {
        printf("blescanswlist=action,index,tag\n");
        return;
    }
    action_val          = strtoul(argv[1], NULL, 10);
    type_val            = strtoul(argv[2], NULL, 10);
    if(argc == 3)
    {
        if(action_val == SCAN_SWLIST_ACTION_CLR) 
            ret = ssv_ble_gap_set_scan_swlist(SCAN_SWLIST_ACTION_CLR, type_val, 0, NULL, 0, mac_type);
    }
    else if(argc == 4)
    {
        index_val           = strtoul(argv[3], NULL, 10);
        if(action_val == SCAN_SWLIST_ACTION_DEL)  
            ret = ssv_ble_gap_set_scan_swlist(SCAN_SWLIST_ACTION_DEL, type_val, index_val, NULL, 0, mac_type);
    }
    else if(argc == 5)
    {
        index_val           = strtoul(argv[3], NULL, 10);
        ...
        if(action_val == SCAN_SWLIST_ACTION_ADD && type_val == SCAN_SWLIST_TAGS_NAME) 
        {
            ret = ssv_ble_gap_set_scan_swlist(0, SCAN_SWLIST_TAGS_NAME, index_val, (uint8_t*)tag_str, strlen(tag_str), mac_type);
        }
    }
    else if(argc == 6)
    {
        index_val           = strtoul(argv[3], NULL, 10);
        tag_str             = argv[4];
        mac_type            = strtoul(argv[5], NULL, 10);
		
        if(action_val == SCAN_SWLIST_ACTION_ADD && type_val == SCAN_SWLIST_TAGS_MAC) 
        {
            for (i = 0; i < SSV_BD_ADDR_LEN; i++)
                st_remote_bda[SSV_BD_ADDR_LEN-1-i] = strtoul(tag_str+3*i, NULL, 16);

            ret = ssv_ble_gap_set_scan_swlist(0, SCAN_SWLIST_TAGS_MAC, index_val, (uint8_t*)&st_remote_bda[0], 6, mac_type);
        }
    }
    if(ret < 0)
        printf("blescanswlist=err\n");
    else
        printf("blescanswlist=ok\n");
}
...
static void cmd_help(int argc, char *argv[])
{
    ...
    printf("scanswlist - ble set scan swlist, filter by name or mac\n");
    printf("\033[1;31meg. scanswlist=0,0,0,demo\033[0m\n");
    printf("\033[1;36m    arg0:action(0:add, 1:del,2:clear)\033[0m\n");
    printf("\033[1;36m    arg1:type(0:name, 1:mac)\033[0m\n");
    printf("\033[1;36m    arg2:index(0-5)\033[0m\n");
    printf("\033[1;36m    arg3:name or mac string\033[0m\n");
	printf("\033[1;36m    arg4:mac type(if set type=1)\033[0m\n\n");
...
#endif
    printf("startservice - start or stop gatt service\n");
    printf("\033[1;31meg. startservice=10(service handle),0(disable)\033[0m\n");
...
void cmd_startservice(int argc, char *argv[])
{
    //ssv_bd_addr_t st_remote_bda;
    uint16_t service_handle, start_stop;

    if (argc < 2) {
        printf("startservice=service handle,start or stop\n");
        return;
    }

    service_handle    = strtoul(argv[1], NULL, 10);
    start_stop   = strtoul(argv[2], NULL, 10);

    if (start_stop)
        ssv_ble_gatts_start_service(service_handle);
    else
        ssv_ble_gatts_stop_service(service_handle);
