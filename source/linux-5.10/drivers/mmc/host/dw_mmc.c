...
static u32 dw_mci_prep_stop_abort(struct dw_mci *host, struct mmc_command *cmd)
{
	...
	if (cmdr == MMC_READ_SINGLE_BLOCK ||
		...
		stop->arg |= ((u32)0x1 << 31) | (0 << 28) | (SDIO_CCCR_ABORT << 9) |
			     ((cmd->arg >> 28) & 0x7);
...
static inline void dw_mci_set_cto(struct dw_mci *host)
{
	...
	/* except for CLKDIV,the default phase will be divide by 2 */
	cto_div = (mci_readl(host, CLKDIV) & 0xff) * 4;
	if (cto_div == 0)
		cto_div = 2;
...
static void dw_mci_set_drto(struct dw_mci *host)
{
	...
	/* except for CLKDIV,the default phase will be divide by 2 */
	drto_div = (mci_readl(host, CLKDIV) & 0xff) * 4;
	if (drto_div == 0)
		drto_div = 2;
...
int dw_mci_probe(struct dw_mci *host)
{
	...
	/* set UHS_EXT clock division factor to 1,so that we
	 * only need to adjust CLK_DIV to divide the clock.
	 */
	mci_writel(host, UHS_REG_EXT, SDMMC_UHS_EXT_CLK((u32)0x2));
