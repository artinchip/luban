...
#if defined(CONFIG_USB_AIC_UDC) || defined(CONFIG_USB_DWC2)
void sourcesink_set_alt_work(struct work_struct *work)
{
	struct f_sourcesink *ss = container_of(work, struct f_sourcesink,
						work.work);
	struct usb_function *f = &ss->function;
	struct usb_composite_dev *cdev = f->config->cdev;
	int ret = 0;
	unsigned int alt = ss->alt;
		...
		unsigned intf, unsigned alt)
{
	...
	if (alt != 0x0) {
		ss->alt = alt;
		ret = schedule_delayed_work(&ss->work, HZ / 2);
		DBG(cdev, "schedule_delayed_work ret =%d.\n", ret);
	}
	return 0;
#endif
...
#if defined(CONFIG_USB_AIC_UDC) || defined(CONFIG_USB_DWC2)
	INIT_DELAYED_WORK(&ss->work, sourcesink_set_alt_work);
#endif
