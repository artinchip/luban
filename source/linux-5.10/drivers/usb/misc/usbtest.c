...
#if defined(CONFIG_USB_AIC_UDC) || defined(CONFIG_USB_DWC2)
static int global_alt;
#endif
...
#if defined(CONFIG_USB_AIC_UDC) || defined(CONFIG_USB_DWC2)
	for (tmp = intf->num_altsetting - 1; tmp >= 0; tmp--) {
#else
...
found:
	...
	if (alt->desc.bAlternateSetting != 0) {
		...
		if (tmp < 0)
			return tmp;
#if defined(CONFIG_USB_AIC_UDC) || defined(CONFIG_USB_DWC2)
		global_alt =  alt->desc.bAlternateSetting;
		pr_debug("%s set global_alt = %d", __func__, global_alt);
#endif
	}
...
usbtest_ioctl(struct usb_interface *intf, unsigned int code, void *buf)
{
	...
	/* interrupt endpoint (not implement) */
	case 25:
	case 26:
		if (global_alt != 2) {
			sw = 1;
	...
	pr_debug("%s test_num = %d, sw = %d, des_alt = %d",
		 __func__, param_32->test_num, sw, des_alt);
	if (sw) {
		retval = set_altsetting(dev, des_alt);
		if (retval) {
			dev_err(&intf->dev,
				"set altsetting to %d failed, %d\n",
				dev->info->alt, retval);
