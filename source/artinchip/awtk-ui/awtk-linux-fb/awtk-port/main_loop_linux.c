...
#ifndef TOUCH_DEVICE_FILENAME
#define TOUCH_DEVICE_FILENAME "/dev/input/event0"
#endif /*MICE_DEVICE_FILENAME*/
...
#endif /*HAS_TSLIB*/
  } else if (tk_str_eq(info->type, "touch")) {
    printf("touch_thread_run create \n");
    thread = touch_thread_run(info->path, input_dispatch_to_main_loop, loop, loop->w, loop->h);
  }
