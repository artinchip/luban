...
static lcd_t* lcd_linux_create_swappable(fb_info_t* fb) {
  ...
  online_buffer[0].buf = fb->fbmem0;
  online_buffer[0].phy_addr = fb->fix.smem_start;
  ...
  if (fb_is_2fb(fb)) {
    ...
    online_buffer[1].buf = fb->fbmem0 + (line_length * h);
    online_buffer[1].phy_addr = fb->fix.smem_start + (line_length * h);
...
static lcd_t* lcd_linux_create(fb_info_t* fb) {
  ...
  if (fb_is_1fb(fb)) {
    log_debug("enter lcd_linux_create_flushable\n");
    return lcd_linux_create_flushable(fb);
  } else {
    log_debug("enter lcd_linux_create_swappable\n");
