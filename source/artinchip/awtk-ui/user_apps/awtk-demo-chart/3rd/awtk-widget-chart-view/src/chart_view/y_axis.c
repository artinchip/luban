...
ret_t y_axis_measure_series(widget_t* widget, void* measure_params, object_t* src, object_t* dst) {
  int32_t i;
  int32_t nr;
  int32_t index;
  ...
  if (tk_abs(sinterval) >= 1.0) {
    for (i = 0; i < nr; i++) {
      ...
      params->draw_data_set(series_fifo_get(dst, SERIES_FIFO_GET_SIZE(dst) - 1), x, src, index,
...
ret_t y_axis_draw_tick(axis_t* axis, canvas_t* c) {
  ...
  image_draw_type_t draw_type = (image_draw_type_t)style_get_int(
      style, STYLE_ID_AXIS_TICK_IMAGE_DRAW_TYPE, IMAGE_DRAW_PATCH3_Y);
...
ret_t y_axis_draw_split_line(axis_t* axis, canvas_t* c) {
  ...
  image_draw_type_t draw_type = (image_draw_type_t)style_get_int(
      style, STYLE_ID_AXIS_SPLIT_LINE_IMAGE_DRAW_TYPE, IMAGE_DRAW_PATCH3_Y);
...
ret_t y_axis_on_paint_before(widget_t* widget, canvas_t* c) {
  ...
  if (axis->data != NULL) {
    if (axis->split_line.show) {
      y_axis_on_paint_begin(widget, c);
