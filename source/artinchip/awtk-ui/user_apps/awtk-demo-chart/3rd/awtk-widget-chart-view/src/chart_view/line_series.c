...
ret_t line_series_get_prop(widget_t* widget, const char* name, value_t* v) {
...
ret_t line_series_set_prop_internal(widget_t* widget, const char* name, const value_t* v) {
  line_series_t* series = LINE_SERIES(widget);
  return_value_if_fail(series != NULL && name != NULL && v != NULL, RET_BAD_PARAMS);
...
static ret_t line_series_set_value(widget_t* widget, const char* value) {
...
static ret_t line_series_set_prop(widget_t* widget, const char* name, const value_t* v) {
  ...
  if (ret == RET_NOT_FOUND) {
    if (tk_str_eq(name, WIDGET_PROP_VALUE)) {
      return line_series_set_value(widget, value_str(v));
    }
  }
...
ret_t line_series_on_destroy(widget_t* widget) {
  line_series_t* series = LINE_SERIES(widget);
  ...
  if (series->line.show || series->area.show) {
    ...
    if (series->line.show) {
      if (series->line.smooth) {
        draw_smooth_line(widget, vg, widget->astyle, ox, oy, fifo, index, size, vertical);
      } else {
        draw_line(widget, c, vg, widget->astyle, ox, oy, fifo, index, size);
      }
    }

    if (series->area.show) {
      if (series->line.smooth) {
        draw_smooth_area(widget, vg, widget->astyle, ox, oy, fifo, index, size, vertical);
      } else {
        draw_area(widget, vg, widget->astyle, ox, oy, fifo, index, size, vertical);
      }
    }
...
ret_t line_series_on_paint_self(widget_t* widget, canvas_t* c) {
  line_series_t* series = LINE_SERIES(widget);
  return_value_if_fail(series != NULL, RET_BAD_PARAMS);
...
widget_t* line_series_cast(widget_t* widget) {
  return_value_if_fail(
      WIDGET_IS_INSTANCE_OF(widget, line_series) || widget_is_line_series_colorful(widget), NULL);
