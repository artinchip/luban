#include "awtk.h"
#include "navigator.h"
extern ret_t window_bar_init(widget_t* win, void* ctx);
extern ret_t window_line_init(widget_t* win, void* ctx);
extern ret_t window_pie_init(widget_t* win, void* ctx);
extern ret_t window_meter_init(widget_t* win, void* ctx);
extern ret_t home_page_init(widget_t* win, void* ctx);
extern ret_t system_bar_init(widget_t* win, void* ctx);

static ret_t navigator_window_init(const char* name, widget_t* win, void* ctx) {
    if (tk_str_eq(name, "window_bar")) {
      return window_bar_init(win, ctx);
    } else if (tk_str_eq(name, "window_line")) {
      return window_line_init(win, ctx);
    } else if (tk_str_eq(name, "window_pie")) {
      return window_pie_init(win, ctx);
    } else if (tk_str_eq(name, "window_meter")) {
      return window_meter_init(win, ctx);
    } else if (tk_str_eq(name, "home_page")) {
      return home_page_init(win, ctx);
    } else if (tk_str_eq(name, "system_bar")) {
    return system_bar_init(win, ctx);
  }
  return RET_OK;
}

static ret_t navigator_window_open_and_close(const char* name, widget_t* to_close, void* ctx) {
  ret_t ret = RET_OK;
  ...
  if (widget_is_dialog(win) && widget_get_prop_bool(win, NAVIGATOR_PROP_DIALOG_TO_MODAL, TRUE)) {
