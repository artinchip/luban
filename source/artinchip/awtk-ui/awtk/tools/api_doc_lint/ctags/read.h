...
typedef struct sInputFile {
	...
	filepos_t   filePosition;  /* file position of current line */
...
/* State of input Buffer -- hook by joyer
 */
typedef struct sInputBuffer {
  const char *start;                    /* start address of buffer */
  const char *current;                  /* current address of buffer */
  size_t len;                     /* length of buffer */
  boolean active;                 /* active state of buffer */
} inputBuffer;
/*
*   GLOBAL VARIABLES
*/
extern CONST_FILE inputFile File;
extern inputBuffer Buffer;
/*
*   FUNCTION PROTOTYPES
*/
...
extern char *readSourceLine (vString *const vLine, filepos_t location, long *const pSeekValue);
