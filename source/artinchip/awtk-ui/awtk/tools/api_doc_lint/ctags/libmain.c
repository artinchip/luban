...
#include<setjmp.h> /*add to support safeexit() -- hook by joyer*/
jmp_buf ctags_exit_jb; /*safe exit jumpbuf*/
...
/* ctags lib implementation  -- hook by joyer
 * using setjmp to prevent unwanted exit
 * using init/fini/parse to seperate the tags generating process
 */
cookedArgs *ctags_cooked_args = NULL; /*common cooked arguments -- hook by joyer*/


int fini_ctags(void)
{
  /*  Clean up. should be no throw
   */
  if (ctags_cooked_args != NULL)
    cArgDelete (ctags_cooked_args);
  freeKeywordTable ();
...
int ctags_parse_file(const char *filename)
{
  int ex_code;

  ex_code = setjmp(ctags_exit_jb);
  if (ex_code<0){
    perror("can't setjmp");
    return 1;
  }
  if (ex_code==0){ /*normal code */
    *((boolean*)&Option.filter) = TRUE; /*tricky try to write a const value*/
    parseFile(filename);
    return 0;
  }
  else{
    perror("error in ctags_parse_file");
    return 1;
  }
}

int ctags_parse_buffer(const char *filename, const char *buf, size_t len)
{
  int ex_code;
  ...
  if (ex_code==0){ /*normal code */
    *((boolean*)&Option.filter) = TRUE; /*tricky try to write a const value*/
    ...
    Buffer.active = FALSE;
    perror("error in ctags_parse_buffer");
    return 1;
  }
}

/* the safe version exit
 * long jump back to top call stack
 */
void ctags_safe_exit(int ex_code){
  longjmp(ctags_exit_jb, ex_code);
}

extern int ctags_reg_callback(EntryCallbackFunc, void *);
static ctags_export_t ctags_export_functions;
ctags_export_t * get_ctags(EntryCallbackFunc callback, void * callback_param)
{
  int ex_code;
  static char *argvi[] = {"ctags", "-n", "--tag-relative=yes", NULL};
  char **argv=argvi;
  ctags_reg_callback(callback, callback_param);

  Buffer.active = FALSE; /*buffer reading*/
  ex_code = setjmp(ctags_exit_jb);
  if (ex_code<0){
    perror("can't setjmp");
    return NULL;
  ...
  if (ex_code==0){ /*normal code */
    ...
    ctags_cooked_args = cArgNewFromArgv (argv);
    previewFirstOption (ctags_cooked_args);
    testEtagsInvocation ();
    initializeParsing ();
    initOptions ();
    readOptionConfiguration ();
    verbose ("Reading initial options from command line\n");
    parseOptions (ctags_cooked_args);
    checkOptions ();
    {
		ctags_export_functions.close_ctags = fini_ctags;
		ctags_export_functions.parse_file  = ctags_parse_file;
		ctags_export_functions.parse_buffer= ctags_parse_buffer;
		return &ctags_export_functions;
	}
  }
  else{
    perror("error in init_ctags");
    return NULL;
  }
}
