...
typedef struct sTokenInfo {
	...
	filepos_t     filePosition;  /* file position of line containing name */
} tokenInfo;
...
typedef struct sStatementInfo {
    ...
    vString*    type[3];        /* type name --- wjs*/
    vString*    functionPreffix; /* 函数前缀 */
    vString*    preffixCache;
    boolean     gotType;        /* 变量类型已经确定 */
    boolean     isFunPtr;       /* was a function pointer? */
	int		    contextState;
...
static kindOption CKinds [] = {
  ...
  { TRUE,  'i', "include",    "including file directives"},
  { TRUE,  'z', "comment",    "comments"},
};
...
static void reinitStatement (statementInfo *const st, const boolean partial)
{
    ...
    if (! partial) 
    {
        vStringClear(st->type[0]);
        vStringClear(st->type[1]);
		vStringClear(st->type[2]);
							...
							vString *const scope, vString *const typeRef)
{
    ...
    if (st->isFunPtr && vStringLength (Signature) > 0) {
        tag->extensionFields.signature = vStringValue (Signature);
        tag->functionPreffix = vStringValue(st->functionPreffix);
    }
...
static void addVariableType(const statementInfo *const st, tagEntryInfo* e, const tagType type)
{
    switch(type)
    {
    case TAG_EXTERN_VAR:
...
// pushVariableType 和 popVariableType
// 均通过 改变 st->type[0 1 2] 的指向而实现
// 不要丢失 st->type 的指向， 以免内存泄漏

static void pushVariableType(statementInfo *const st, const vString* const name)
{
	vString* s = st->type[0]; 
	vStringCopy(s, name);
...
static void popVariableType(statementInfo *const st)
{
	vString* s = st->type[2];
	vStringClear(s);
...
static boolean isBuildType(keywordId id)
{
    switch (id)
    {
    case KEYWORD_FLOAT:
...
static void updateVariableType(statementInfo *const st, const tokenInfo *const token)
{
    if (st->gotType) return;

    if (isType (token, TOKEN_NAME) || isBuildType(token->keyword))
        pushVariableType(st, token->name);
...
static void  updateFunctionPreffix(statementInfo *const st, const tokenInfo *const token)
{
    ...
    if ( isType(token, TOKEN_KEYWORD) || isType (token, TOKEN_NAME) ) 
        pushFunctionPreffix(st, vStringValue(token->name));
}
					 ...
					 boolean isFileScope, const tagType type)
{
		...
		includeTag (type, isFileScope))
	{
        ...
        if (e.kind != 's' && e.kind != 'c')
            *(boolean*)&(st->gotType) = TRUE; /*tricky try to write a const value*/
        addVariableType(st, &e, type);
...
{
	tokenInfo *const token = activeToken (st);
	tokenInfo *const prev = prevToken(st, 2);
	const unsigned long line = prev ? prev->lineNumber : -1;
			...
			if (isident1 (c))
			{
				...
				default:
					...
					else
					{
						 ...
						 *  这个是类
						 */
                        //if ( !st->gotType &&  vStringLength(st->type[1]) < 1)
							...
							st->declaration == DECL_UNION)
                        {
						 ...
						 /* 
                         * void fun(int arg) something1 something2 ...
                         * {}
                         * 这个是函数
						 * 
						 *	 BEGIN_MESSAGE_MAP(KFCMainFrame, CPropertySheet)
						 *	 //{{AFX_MSG_MAP(KFCMainFrame)
						 *	 ON_WM_SYSCOMMAND()
						 *	 //}}AFX_MSG_MAP
						 *	 END_MESSAGE_MAP()
						 *
						 *	 BOOL Dlg::OnInitDialog() { ... }
						 *   防止 END_MESSAGE_MAP() 干扰 而解析不出 OnInitDialog 函数
						 */
...
static int parseParens (statementInfo *const st, parenInfo *const info)
{
	...
	if (info->isKnrParamList && info->parameterCount > 0)
		vStringClear (Signature);
...
static void clearContext(statementInfo *const st)
{
	vStringClear(st->context->name);
}
...
static statementInfo *newStatement (statementInfo *const parent)
{
    ...
    st->type[0] = vStringNew();
    st->type[1] = vStringNew();
	st->type[2] = vStringNew();
    st->functionPreffix = vStringNew();
    st->preffixCache = vStringNew();
...
static void deleteStatement (void)
{
    ...
    vStringDelete(st->type[0]);            st->type[0] = NULL;
    vStringDelete(st->type[1]);            st->type[1] = NULL;
	vStringDelete(st->type[2]);			   st->type[2] = NULL;
    vStringDelete(st->functionPreffix);    st->functionPreffix = NULL;
    vStringDelete(st->preffixCache);       st->preffixCache = NULL;
	eFree (st);
...
static void tagCheck (statementInfo *const st)
{
    ...
    updateFunctionPreffix(st, token);
	...
	switch (token->type)
	{
		...
		case TOKEN_BRACE_OPEN:
					...
					st->declaration == DECL_PROGRAM)
			{
                ...
                findScopeHierarchy(s, st);
                if (vStringLength(s) > 0)
                    vStringCatS(s, "::");
                vStringCat(s, st->blockName->name);
                pushVariableType(st, s);
                vStringDelete(s);
			}
			else if (isLanguage (Lang_csharp))
