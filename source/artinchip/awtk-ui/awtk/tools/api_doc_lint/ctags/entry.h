...
typedef struct sTagEntryInfo {
	int         lineNumberEntry;  /* pattern or line number entry */
	unsigned long lineNumber;     /* line number of tag */
	filepos_t   filePosition;     /* file position of line containing tag */
	const char* language;         /* language of source file */
	int         isFileScope;      /* is tag visibile only within source file? */
	int         isFileEntry;      /* is this just an entry for a file name? */
	int         truncateLine;     /* truncate tag line at end of tag name? */
	const char *sourceFileName;   /* name of source file */
	const char *name;             /* name of the tag */
	const char *kindName;         /* kind of tag */
  const char *type;
	const char *functionPreffix;
