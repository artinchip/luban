...
*   DATA DEFINITIONS
*/
inputBuffer Buffer; /* globally read buffer -- hook by joyer */
inputFile File;  /* globally read through macros */
static filepos_t StartOfLine;  /* holds deferred position of start of line */
...
/*single char read from buffer -- hook by joyer
 */
static int iBufferGetc(void)
{
	int	c;
	c = (Buffer.current>=Buffer.start+Buffer.len) ? EOF : *Buffer.current++;
	...
	{
		fileNewline ();
		Buffer.current = Buffer.start + StartOfLine;
        c = (Buffer.current>=Buffer.start+Buffer.len) ? EOF : *Buffer.current++;
	}
	...
	{
		File.newLine = TRUE;
		StartOfLine = Buffer.current - Buffer.start;
	}
	else if (c == CRETURN)
	{
      /* is CR followed by LF? */
		const int next = (Buffer.current>=Buffer.start+Buffer.len) ? EOF : *Buffer.current++;
        
		if (next != NEWLINE  && next != EOF)
          Buffer.current--;
...
{
	int	c;
    if (Buffer.active)/*read by buffer-- hook by joyer*/
      return iBufferGetc();
readnext:
	c = getc (File.fp);
...
extern char *readSourceLine (
  vString *const vLine, filepos_t location, long *const pSeekValue)
{
	filepos_t orignalPosition;
	char *result;
