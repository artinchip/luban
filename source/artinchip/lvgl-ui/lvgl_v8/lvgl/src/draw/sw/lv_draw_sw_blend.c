...
#if defined(USE_AIC_MEM) && !defined(USE_CACHE_FRAMEBUFFER)
...
#if defined(USE_AIC_MEM) && !defined(USE_CACHE_FRAMEBUFFER)
extern bool is_fb_addr(char *buf);
static bool is_fb = false;

static inline void aic_fb_memory(void *dst, const void *src, unsigned long count)
{
    if (is_fb)
        aic_dmabuf_memcpy(dst, src, count);
    else
        lv_memcpy(dst, src, count);
}
                                            ...
                                            lv_coord_t dest_stride, lv_color_t color, lv_opa_t opa, const lv_opa_t * mask, lv_coord_t mask_stride)
{
    ...
    if(mask == NULL) {
        if(opa >= LV_OPA_MAX) {
            for(x = 0; x < w; x++) {
#if defined(USE_AIC_MEM) && !defined(USE_CACHE_FRAMEBUFFER)
                aic_fb_memory(dest_buf8, ctmp, LV_IMG_PX_SIZE_ALPHA_BYTE);
#else
            ...
            for(y = 1; y < h; y++) {
#if defined(USE_AIC_MEM) && !defined(USE_CACHE_FRAMEBUFFER)
                aic_fb_memory(dest_buf8, (uint8_t *) dest_buf, w * LV_IMG_PX_SIZE_ALPHA_BYTE);
#else
...

{
    ...
    if(mask == NULL) {
        if(opa >= LV_OPA_MAX) {
            for(y = 0; y < h; y++) {
#if defined(USE_AIC_MEM) && !defined(USE_CACHE_FRAMEBUFFER)
                aic_fb_memory(dest_buf, src_buf, w * sizeof(lv_color_t));
#else
