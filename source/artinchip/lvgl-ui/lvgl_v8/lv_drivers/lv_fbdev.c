...
bool is_fb_addr(char *buf)
{
#ifdef USE_DRAW_BUF
    return 1;
#else
    if (buf == g_frame_buf[0] || buf == g_frame_buf[1])
        return true;
    else
        return false;
#endif
...
#ifdef USE_CACHE_FRAMEBUFFER
    if (ioctl(g_fb, AICFB_UPDATE_DMA_COHERENT_STATUS, (bool[]){ false }) < 0) {
        LV_LOG_ERROR("Error: failed to switch framebuffer dma coherent status");
        return -1;
    }

    g_fb_len = vinfo.yres * finfo.line_length;
    struct dma_buf_info fds;
    if (ioctl(g_fb, AICFB_TO_DMABUF_FD, &fds) < 0) {
        LV_LOG_ERROR("export aicdb to dmabuf fd failed");
        return -1;
    }
...
void fbdev_buf_sync(unsigned char *buf)
{
    ...
    if (g_fb_dmabuf_fd > 0) {
        sync.start = (unsigned long)buf;
        sync.size = g_fb_len;
        sync.flags = DMA_BUF_SYNC_WB_RANGE;
        if (ioctl(g_fb_dmabuf_fd, DMA_BUF_IOCTL_SYNC_RANGE, &sync) < 0)
            LV_LOG_ERROR("ioctl dmabuf_sync_range fail");
    }
}
#endif
