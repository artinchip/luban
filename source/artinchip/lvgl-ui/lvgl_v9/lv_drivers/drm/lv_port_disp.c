...
static int find_plane(drm_dev_t * drm_dev, unsigned int fourcc,
		      uint32_t * plane_id, uint32_t crtc_id, uint32_t crtc_idx);
...
int lv_linux_drm_set_file(drm_dev_t * drm_dev, const char * file, int64_t connector_id)
{
	int ret;
...
void lv_port_disp_init(void)
{
	...
	lv_linux_drm_set_file(drm_dev, "/dev/dri/card0", -1);
}
