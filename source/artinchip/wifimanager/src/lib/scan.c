	...
	while(1) {
		...
		if(ret) {
			if(strncmp(reply, "FAIL-BUSY", 9) == 0) {
				wifimanager_debug(MSG_DEBUG, "wpa_supplicant is scanning internally\n");
				event = WPAE_SCAN_RESULTS;
				/*Wpa_supplicant is scanning internally , so get he scan results directly*/
				goto scan_resluts;
			} else {
				wifimanager_debug(MSG_DEBUG, "send scan error:%s\n", reply);
				return -1;
			}
		}
...
read_event:
		...
		if((event == WPAE_DISCONNECTED) || (event == WPAE_NETWORK_NOT_FOUND)) {
			wifimanager_debug(MSG_WARNING, "read event again......\n");
			goto read_event;
		}

		/*send scan command failed , try to scan again.*/
		if(event == WPAE_SCAN_FAILED) {
			wifimanager_debug(MSG_WARNING, "scan again......\n");
			scan.try_scan_count ++;
			if(scan.try_scan_count > TRY_SAN_MAX) {
				wifimanager_debug(MSG_WARNING, "send scan cmd failed\n");
				return -1;
			}
			sleep(1);
			continue;
...
scan_resluts:
		if(event == WPAE_SCAN_RESULTS) {
			...
			if(ret) {
				wifimanager_debug(MSG_ERROR, "do scan results error!\n");
				return -1;
			}
			remove_slash_from_scan_results();
			scan.results_len =  strlen(scan.results);
		} else {
			wifimanager_debug(MSG_ERROR, "read scan data is failed\n");
			return -1;
		}
		...
		if(*len <= scan.results_len) {
			wifimanager_debug(MSG_WARNING, "Scan result overflow, %d < %d\n", *len, scan.results_len);
...
int get_key_mgmt(const char *ssid, int key_mgmt_info[])
{
	...
	wifimanager_debug(MSG_DEBUG, "get_key_mgmt, ssid %s\n", ssid);
	...
	if(direct_get_scan_results_inner(NULL, NULL) != 0) {
		wifimanager_debug(MSG_WARNING, "get scan result is null\n");
		return 0;
	}
	...
	if(len <= 48) {
		wifimanager_debug(MSG_ERROR, "get scan results is null\n");
		return 0;
	}
	...
	while(1) {
		...
		if(strcmp(pssid, ssid) == 0) {
			...
			flag[len] = '\0';
			wifimanager_debug(MSG_DEBUG, "ssid %s, flag %s\n", ssid, flag);
