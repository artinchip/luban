...
import os, sys, subprocess, math, re, zlib, zipfile, tarfile, json, struct, argparse
import pinmux_check
...
def itb_create_image(itsname, itbname, keydir, dtbname):
    ...
    if keydir != None and dtbname != None and SIGN:
...
def uboot_env_create_image(srcfile, outfile, size, part_str, redund):
...
def firmware_component_preproc(cfg, datadir, keydir):
    ...
    if "hash_table" in preproc_cfg and SIGN:
        ...
        for name in imgnames:
            ...
            data = run_shell(["veritysetup", "format", "--data-block-size={}".format(datasize), "--hash-block-size={}".format(hashsize), rootfsfile, outfile])
    ...
    if "uboot_env" in preproc_cfg:
        ...
        for binname in imgnames:
            ...
            if srcfile == None:
                ...
                sys.exit(1)
            uboot_env_create_image(srcfile, outfile, envsize, part_str, envredund)
    ...
    if "extension" in preproc_cfg and not SIGN:
...
def build_firmware_image(cfg, datadir):
    ...
    if BURNER:
        os.makedirs(datadir + "burner/", exist_ok=True)
        img_bin_fn = datadir + "burner/" + img_gen_fw_file_name(cfg).replace(".img", ".bin")
...
def extract_img_data(img):
    datadir = os.path.dirname(img) + Path(img).stem
